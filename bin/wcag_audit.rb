...
PAGES.each_with_index do |p, i|
  ...
  issues = Array(result)
  errors = issues.count { |i_| i_["type"] == "error" }
  warns  = issues.count { |i_| i_["type"] == "warning" }
  notes  = issues.count { |i_| i_["type"] == "notice" }
  puts "#{errors}E / #{warns}W / #{notes}N"
  results << { page: p, issues: issues, errors: errors, warnings: warns, notices: notes }
end
...
# Summary table
out << "## Summary\n\n"
out << "| Page | URL | 🔴 errors | 🟡 warnings | 🔵 notices |\n"
out << "|---|---|---:|---:|---:|\n"
total_e = total_w = total_n = 0
results.each do |r|
  next if r[:error]
  out << "| #{r[:page][:label]} | `#{r[:page][:path]}` | #{r[:errors]} | #{r[:warnings]} | #{r[:notices]} |\n"
  total_e += r[:errors]; total_w += r[:warnings]; total_n += r[:notices]
end
out << "| **Totals** | | **#{total_e}** | **#{total_w}** | **#{total_n}** |\n\n"
...
puts "→ Totals: #{total_e}E / #{total_w}W / #{total_n}N across #{results.count { |r| !r[:error] }} pages"

...
# Issue #548 — meta-refresh trap bailout. Documents stuck in pending /
# processing past SparcConfig.processing_stuck_minutes must stop emitting
# the auto-refresh tag and show a "stuck" message instead, so the browser
# tab is no longer trapped in a 3-second redirect loop.
RSpec.describe "shared/_processing_banner.html.erb", type: :view do
  ...
  context "when the document is fresh (within the stuck threshold)" do
    ...
    it "emits the meta-refresh tag" do
      render partial: "shared/processing_banner", locals: locals
      expect(rendered).to include(%(<meta http-equiv="refresh"))
  ...
  context "when the document has been stuck past the threshold" do
    ...
    it "does NOT emit the meta-refresh tag" do
      render partial: "shared/processing_banner", locals: locals
      expect(rendered).not_to include(%(<meta http-equiv="refresh"))
    end
  ...
  context "when the document has explicitly failed" do
    ...
    it "does NOT emit the meta-refresh tag (existing behavior)" do
      render partial: "shared/processing_banner", locals: locals
      expect(rendered).not_to include(%(<meta http-equiv="refresh"))

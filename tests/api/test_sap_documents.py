...
def _new_payload() -> dict[str, Any]:
    # SAP only requires name; description is included for completeness.
    return make_payload(PARAM_KEY)


@pytest.fixture
def sap_doc(admin_client: httpx.Client) -> Iterator[dict[str, Any]]:
...
class TestCreate:
    ...
    def test_create_round_trip(self, admin_client: httpx.Client) -> None:
        ...
        assert_create_round_trip(
            admin_client, PATH, _new_payload(), PARAM_KEY, SapDocumentShow
        )

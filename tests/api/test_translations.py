...
instance with the `hdf` binary on PATH. The amendments test chains off the
poam_from_hdf output rather than committing a second OSCAL fixture.
"""
...
class TestPoamFromHdf:
    ...
    def test_raw_body_returns_oscal_poam(self, admin_client: httpx.Client) -> None:
        ...
        assert response.status_code == 200, response.text
        body = response.json()
        assert "plan-of-action-and-milestones" in body, body
...
class TestAmendmentsFromOscalPoam:
    @pytest.mark.happy
    def test_oscal_poam_round_trips_to_amendments(self, admin_client: httpx.Client) -> None:
        # Chain: produce an OSCAL POAM from the HDF fixture, then translate
        # it back to HDF Amendments. Avoids committing a separate OSCAL
        # fixture and proves the round-trip the controller guards with
        # `hdf amend verify`.
        poam = _post_raw(admin_client, POAM_PATH, _hdf_bytes())
        assert poam.status_code == 200, poam.text

        response = _post_raw(admin_client, AMENDMENTS_PATH, json.dumps(poam.json()).encode())
        assert response.status_code == 200, response.text

...
The coverage spine is [`docs/api/INVENTORY.md`](../../docs/api/INVENTORY.md), regenerated by `bin/api_inventory_check.rb --write`. **Not every row in that table has a test here** — as of 2026-08-20, 38 of 208 endpoints map to no module at all, and the column that says they do counts module presence rather than verification. Read the summary block in `INVENTORY.md` for what each column proves; closing the gap is [#995](https://github.com/risk-sentinel/sparc/issues/995).
...
This is the shape a module is written to, **not a description of what the suite currently holds**. Measured on 2026-08-20 the estate runs 4:1 in favour of happy paths, and several modules assert a status code without ever reading the response body — which is the exact shape that let [#994](https://github.com/risk-sentinel/sparc/issues/994) answer `200 {"status":"updated"}` to a body it never parsed. For every endpoint in `INVENTORY.md`, a module should carry at minimum:
...
**There is none, and this section used to claim otherwise.** It described a workflow at
`.github/workflows/api-tests.yml` that boots the stack, seeds it, mints tokens, runs the suite and
computes a coverage delta against `INVENTORY.md`. No such file exists, and no workflow runs this
suite: `ci.yml` only lints it with `ruff`. The 451 test functions here execute solely when a
developer runs them locally as the pre-push gate described in `docs/dev/issue_rules.md`.

That matters more than a stale paragraph normally would, because this suite is the evidence behind
the release gate in [#995](https://github.com/risk-sentinel/sparc/issues/995) — evidence a
contributor could reasonably believe CI was collecting on every pull request.

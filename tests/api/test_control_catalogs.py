...
class TestReviewWorkflow:
    ...
    failure renders ``{error}`` with the transition's status code. Catalogs
    are id-addressed (reads open to any user; approve/reject are admin-only).
    """
    ...
    def test_submit_then_approve(
        ...
        cid = catalog["id"]
        submit = admin_client.post(f"{PATH}/{cid}/submit_for_review")
        assert submit.status_code == 200, submit.text
        assert submit.json()["data"]["approval_status"]

        approve = admin_client.post(f"{PATH}/{cid}/approve")
    ...
    def test_submit_then_reject_records_reason(
        ...
        cid = catalog["id"]
        admin_client.post(f"{PATH}/{cid}/submit_for_review")
        reject = admin_client.post(
            f"{PATH}/{cid}/reject", json={"reason": "needs more detail"}
        )
    ...
    def test_non_admin_cannot_approve(
        ...
        cid = catalog["id"]
        admin_client.post(f"{PATH}/{cid}/submit_for_review")
        assert user_client.post(f"{PATH}/{cid}/approve").status_code in (401, 403)

    def test_approve_without_submit_is_rejected(
        self, admin_client: httpx.Client, catalog: dict[str, Any]
    ) -> None:
        # A draft that was never submitted for review cannot be approved.
        resp = admin_client.post(f"{PATH}/{catalog['id']}/approve")
        assert resp.status_code in (409, 422), resp.text

    @pytest.mark.auth
    def test_submit_requires_token(
        self, anon_client: httpx.Client, catalog: dict[str, Any]
    ) -> None:
        assert_error_envelope(
            anon_client.post(f"{PATH}/{catalog['id']}/submit_for_review"),
            expected_status=401,
        )

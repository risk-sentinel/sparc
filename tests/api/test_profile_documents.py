...
class TestReviewWorkflow:
    ...
    failure renders ``{error}`` with the transition's status code. Same
    contract exercised for CDEF in test_cdef_documents.py (#642 slice 1).
    """
    ...
    def test_approve_without_submit_is_rejected(
        ...
        resp = admin_client.post(f"{PATH}/{profile_doc['slug']}/approve")
    ...
    def test_submit_requires_token(
        ...
        assert_error_envelope(
            anon_client.post(f"{PATH}/{profile_doc['slug']}/submit_for_review"),
            expected_status=401,
        )

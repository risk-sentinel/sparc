...
- **Every `/api/v1` endpoint was checked against its published contract** ([#995](https://github.com/risk-sentinel/sparc/issues/995)) — all 223, on three axes: the endpoint exists, it is documented, and something exercises it. The sweep found endpoints returning `200` for work they had not done: bulk delete answering `{"deleted": 0}` to a body it never read ([#1018](https://github.com/risk-sentinel/sparc/issues/1018)), baseline parameters reporting success with zero updates ([#994](https://github.com/risk-sentinel/sparc/issues/994)), and `poam_from_amendments` returning schema-**invalid** OSCAL with a `200` ([#1017](https://github.com/risk-sentinel/sparc/issues/1017)).
- **The API reaches what only the browser could.** New surfaces for POA&M items and their remediations, milestones, observations and findings ([#1010](https://github.com/risk-sentinel/sparc/issues/1010)), the converter registry ([#1011](https://github.com/risk-sentinel/sparc/issues/1011)), organizations and membership ([#1012](https://github.com/risk-sentinel/sparc/issues/1012)), service accounts including token regeneration ([#1013](https://github.com/risk-sentinel/sparc/issues/1013)), RBAC roles ([#1014](https://github.com/risk-sentinel/sparc/issues/1014)), leveraged authorizations ([#1015](https://github.com/risk-sentinel/sparc/issues/1015)), API tokens ([#1016](https://github.com/risk-sentinel/sparc/issues/1016)) and file ingestion for CDEF, POA&M, SAP and Profile ([#1031](https://github.com/risk-sentinel/sparc/issues/1031)).
- **The IdP is the system of record for entitlements** ([#860](https://github.com/risk-sentinel/sparc/issues/860), [#842](https://github.com/risk-sentinel/sparc/issues/842), [#707](https://github.com/risk-sentinel/sparc/issues/707)) — OIDC claims map to organization, boundary and role, so an SSO user arrives provisioned rather than waiting on a manual grant, and two divergent role vocabularies are reconciled to one. PIV is accepted as asserted by the IdP via `acr`/`amr` ([#822](https://github.com/risk-sentinel/sparc/issues/822)), so phishing-resistant authentication no longer requires SPARC to terminate the certificate itself. Sessions gained an absolute lifetime ([#1043](https://github.com/risk-sentinel/sparc/issues/1043)) — without one, an active user's entitlements never expired.
- **Authorization gaps now fail a test** ([#919](https://github.com/risk-sentinel/sparc/issues/919)) — every controller was swept for missing checks. CDEF and control-family screens were public regardless of `SPARC_PUBLIC_CATALOGS` ([#974](https://github.com/risk-sentinel/sparc/issues/974)); five CDEF actions had no permission check at all ([#1032](https://github.com/risk-sentinel/sparc/issues/1032)); any valid token could record a KSI validation ([#1024](https://github.com/risk-sentinel/sparc/issues/1024)); and a finding disposition could be approved by the person who set it ([#1034](https://github.com/risk-sentinel/sparc/issues/1034)).
- **OSCAL exports are schema-true.** Regenerating a document minted fresh random UUIDs, churning every identifier downstream ([#957](https://github.com/risk-sentinel/sparc/issues/957)). An SSP carrying provided/responsibility statements could not be exported at all ([#958](https://github.com/risk-sentinel/sparc/issues/958)). Every export embedded every authoritative back-matter resource in the instance ([#959](https://github.com/risk-sentinel/sparc/issues/959)) — now scoped: catalog plus global for a profile, boundary-level for an SSP. The resolved-catalog download is shaped like a conformant resolver's output ([#999](https://github.com/risk-sentinel/sparc/issues/999)), and SPARC now validates against OSCAL **1.2.2** ([#1020](https://github.com/risk-sentinel/sparc/issues/1020)) — 1.2.x tightens constraints, and finding that out from an assessor would be finding out too late. What SPARC *emits* by default deliberately stays 1.1.2.
- **Documents that could not actually be produced, now can.** A CDEF could not be authored from scratch or edited at all ([#944](https://github.com/risk-sentinel/sparc/issues/944)). An SSP generated from a profile had no control statements, so nothing could be leveraged from it ([#955](https://github.com/risk-sentinel/sparc/issues/955)). A SAR generated from an SSP produced an empty POA&M because findings were never converted ([#954](https://github.com/risk-sentinel/sparc/issues/954)). A SAP did not derive its baseline from the selected SSP ([#946](https://github.com/risk-sentinel/sparc/issues/946)). An imported profile could not be linked to a catalog and was then permanently unpublishable ([#928](https://github.com/risk-sentinel/sparc/issues/928)), and re-importing an existing SSP rolled back the whole import ([#963](https://github.com/risk-sentinel/sparc/issues/963)).
- **Parameters and statement prose survive import.** Catalog import truncated statement prose at 200 characters, severing parameter references mid-sentence ([#1003](https://github.com/risk-sentinel/sparc/issues/1003)). Selection choices referencing other ODPs exported as opaque text ([#942](https://github.com/risk-sentinel/sparc/issues/942)), raw `{{ insert: param }}` markup reached the Profile and SSP screens ([#1002](https://github.com/risk-sentinel/sparc/issues/1002)), and a profile's tailored ODP values were invisible wherever a control and its sub-part shared an identifier ([#1007](https://github.com/risk-sentinel/sparc/issues/1007)).
- **Evidence carries its provenance.** Auto-fetched evidence recorded no collector and had no link back to the run that produced it ([#934](https://github.com/risk-sentinel/sparc/issues/934)); an attestation could not be recorded without a file and its attester was unverifiable ([#947](https://github.com/risk-sentinel/sparc/issues/947)); the attester role list went stale when the boundary changed ([#981](https://github.com/risk-sentinel/sparc/issues/981)). The evidence index is tiered Instance → Organization → Boundary ([#948](https://github.com/risk-sentinel/sparc/issues/948)).
- **CDEF coverage from infrastructure you already have** ([#904](https://github.com/risk-sentinel/sparc/issues/904)) — upload a Terraform state or plan and get a needed / have / to-do report. Alongside it, 57% of CCI→NIST resolutions produced a `control_id` matching no catalog control ([#1030](https://github.com/risk-sentinel/sparc/issues/1030)), no STIG-sourced CDEF was ingested end to end by any test ([#1029](https://github.com/risk-sentinel/sparc/issues/1029)), and the AWS Labs refresh reported 39–51 errors on a cold pass while still reporting success ([#939](https://github.com/risk-sentinel/sparc/issues/939)).
- **Boundaries hold their documents.** SSP/SAP/SAR/POA&M could exist with no boundary and were then visible to every signed-in user ([#952](https://github.com/risk-sentinel/sparc/issues/952)); a document could not be attached to a boundary after upload ([#929](https://github.com/risk-sentinel/sparc/issues/929)); a boundary could leverage a system that was never authorized ([#988](https://github.com/risk-sentinel/sparc/issues/988)); and a customer responsibility was auto-marked addressed, so responsibility gaps could not be found ([#956](https://github.com/risk-sentinel/sparc/issues/956)).
- **Tests that could not fail were replaced.** Five credential-redaction tests shared a top-level constant that a later spec overwrote, so they asserted against the wrong value and passed regardless ([#1035](https://github.com/risk-sentinel/sparc/issues/1035)). Twelve collection-view checks skipped silently on screens with no records ([#984](https://github.com/risk-sentinel/sparc/issues/984)), four export services never asserted their validated path ([#989](https://github.com/risk-sentinel/sparc/issues/989)), and the API suite emptied the review queue by editing a seeded fixture ([#1041](https://github.com/risk-sentinel/sparc/issues/1041)).
- **Twenty compliance findings claimed remediation the Debian→UBI9 migration had invalidated** ([#1001](https://github.com/risk-sentinel/sparc/issues/1001)) — still presented as current. Rewritten against what the scanner actually finds in the container, severity included. The hdf-cli pin moves to 3.5.1 ([#993](https://github.com/risk-sentinel/sparc/issues/993)).
- **Stored values render as text, everywhere** ([#897](https://github.com/risk-sentinel/sparc/issues/897)) — every render site was audited for the pattern, not just the two known cases, and user-content downloads are pinned to `disposition: attachment` with a regression test ([#894](https://github.com/risk-sentinel/sparc/issues/894)).
- **The interface stops fighting the window.** The main navbar overflowed the viewport from 992px to ~1400px on every page ([#1042](https://github.com/risk-sentinel/sparc/issues/1042)); 489 buttons carried bespoke colour classes with no light/dark parity, now one role per button across 13 semantic roles ([#950](https://github.com/risk-sentinel/sparc/issues/950)); the sidebar scrolls independently ([#951](https://github.com/risk-sentinel/sparc/issues/951)); nine views set a page title the layout never yielded, so every tab read "SPARC" ([#991](https://github.com/risk-sentinel/sparc/issues/991)); and saved URLs show a real favicon and link preview ([#936](https://github.com/risk-sentinel/sparc/issues/936)).
- **Authoritative sources are editable** ([#1039](https://github.com/risk-sentinel/sparc/issues/1039)) — the library was create-only, so a typo in an href was permanent. Sources now carry provenance and control references, and `DELETE` archives rather than destroys. A reference resolves against a chosen catalog, because with Rev 4 and Rev 5 both loaded a bare "AC-2" names a control in each.
- Also: field-level help on the remaining edit screens ([#879](https://github.com/risk-sentinel/sparc/issues/879)) and an in-page guide drawer ([#880](https://github.com/risk-sentinel/sparc/issues/880)); filter buttons on index screens ([#908](https://github.com/risk-sentinel/sparc/issues/908)); a profile's controls, parameters and guidance are visible and editable ([#997](https://github.com/risk-sentinel/sparc/issues/997)); `framework` derived at import ([#935](https://github.com/risk-sentinel/sparc/issues/935)); statement sub-parts sort under their parent ([#941](https://github.com/risk-sentinel/sparc/issues/941)); mapping entries are picked rather than typed ([#945](https://github.com/risk-sentinel/sparc/issues/945)); a published profile's parameters are lifecycle-guarded ([#1008](https://github.com/risk-sentinel/sparc/issues/1008)); a single `SPARC_BANNER` variable ([#909](https://github.com/risk-sentinel/sparc/issues/909)); `SPARC_RESOURCES` no longer replaces the shipped list wholesale ([#914](https://github.com/risk-sentinel/sparc/issues/914)); and a reusable two-boundary leveraged reference authorization for E2E and DAST ([#845](https://github.com/risk-sentinel/sparc/issues/845)).

**Behaviour changes:** `DELETE /api/v1/authoritative_sources/:id` **archives** and returns the archived record rather than a bare `204` — a client treating it as destructive will be wrong. OSCAL exports no longer embed every authoritative back-matter resource: a profile export carries catalog plus global resources, an SSP export carries boundary-level ones. Sessions now have an **absolute lifetime**, so a long-lived session is ended rather than carrying stale entitlements indefinitely. Documents can no longer be created without a boundary. `SPARC_BANNER` replaces `SPARC_BANNER_HTML` / `SPARC_BANNER_MESSAGE`, which are deprecated but still read — set `SPARC_BANNER` and the old pair is ignored with a warning. `SPARC_RESOURCES` now augments the shipped list rather than replacing it. OSCAL **1.2.2** is added to the schemas SPARC can validate against; `DEFAULT_VERSION` stays **1.1.2**, so the OSCAL you export is unchanged.

[Full release notes](https://github.com/risk-sentinel/sparc/releases/tag/v1.16.0).

...
7. **Auto-fill from CDEFs** (if selected) -- matches CDEF controls to SSP controls by NIST ID, populates `private_implementation` from the CDEF's `implementation_narrative`, upgrades status from "Deferred" to "Implemented" when narrative is present, and creates by-component entries.

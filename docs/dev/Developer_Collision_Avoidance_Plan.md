...
| [ ] | **#466** AWS Labs CDEF runtime ingestion + read-only inventory | Core/CDEF + Infrastructure | `app/services/aws_labs_cdef_import_service.rb` (NEW), `app/services/aws_labs_cdef_source_client.rb` (NEW), `app/jobs/aws_labs_cdef_refresh_job.rb` (NEW), `lib/tasks/aws_labs_cdefs.rake` (NEW), `app/models/cdef_document.rb` (predicates + cloned_from association + aws_labs_sourced scope), `app/models/sparc_config.rb` (env vars), `app/controllers/cdef_documents_controller.rb` (ensure_editable! + copy clone-tracking), `app/controllers/api/v1/cdef_documents_controller.rb` (`?source_type=aws_labs` filter + source field in payload), `app/views/cdef_documents/show.html.erb` (read-only banner + copy button), `config/recurring.yml` (daily 06:00), migration adds `cloned_from_id` FK + partial unique index, `NOTICE` (NEW), `LICENSES/AWS-LABS-OSCAL-CONTENT-LICENSE` (NEW), `docs/compliance/THIRD_PARTY_NOTICES.md` (NEW), compliance mapping (RA-3(1), CA-2, CM-8, SR-3, SA-15) | **MEDIUM** -- new service-layer surface + controller hook; coordinates with existing CdefDocument concerns |

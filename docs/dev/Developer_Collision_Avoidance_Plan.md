...
| [ ] | **#472** SBOM license inventory + policy gate (warn-only initial release) | CI/Infrastructure + Compliance | `.github/workflows/security.yml` (Trivy `--scanners license` on fs+container, new `license_inventory` job, bundle_results + publish_for_sparc_iac wiring), `scripts/ci/build_license_inventory.rb` (NEW), `docs/compliance/license-policy.yml` (NEW), `docs/compliance/license-dispositions.yml` (NEW), `docs/compliance/THIRD_PARTY_NOTICES.md`, `docs/compliance/nist-sp800-53-rev5-mapping.md` (RA-3(1), CA-2, CM-8, SR-3, SA-15, SA-22), `spec/scripts/build_license_inventory_spec.rb` (NEW) | **LOW** -- CI workflow + new policy files only; no application code |
| [ ] | **#473** Aggregator: don't short-circuit on stale failure when in-flight re-run pending | CI/Infrastructure | `.github/workflows/required-passed.yml` (poll-loop logic) | **LOW** -- single workflow file |

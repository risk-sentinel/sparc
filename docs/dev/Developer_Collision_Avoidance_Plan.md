...
| [ ] | **#470** Migration squash — fold 29 post-2026-03-19 migrations into single consolidated file | DB/Infrastructure | `db/migrate/*.rb` (29 files moved to `db/migrate_archive/`), new `db/migrate/<ts>_squash_migrations_to_current_schema.rb` (same pattern as 2026-03-19 squash), `db/schema.rb` (re-stamped) | **HIGH** -- touches every active migration. Coordinate freeze for the duration; merge order: AFTER all in-flight feature PRs, BEFORE patch release tag. |

...
**Last updated:** 2026-09-17 — **v1.16.1 SHIPPED, tagged from `df6439c0`**; all six bundles (Y, Z, AA, AB, AC, AD) delivered — see *Lane REL* in section 2. **Next: `v1.17.0`** (renamed from v1.17.1, which had no `.0` above it), 6 issues, bundles AE/AF proposed and AG unsequenced — see Phase 19 in the Implementation plan. *Previously (2026-08-25):* **two lanes now run concurrently** (`ci.v0.0.1` CI work and `v1.16.1` release work, owner decision 2026-08-25, to protect #968's 09-06 due date). See *Phases 17-18* in section 2 for the lane split and the four crossings between them. CI-1 merged (PR #1066); CI-2 in review (PR #1068).
...
| [x] | **#479** Drop roo-xls + legacy UI scrub + v1.6.2 -- **COMPLETED 2026-05-18** | CI/Infrastructure + UI | (shipped in v1.6.2; PR #480) | **NONE** -- completed |
...
| [x] | #875 | Boundaries/Personnel | `app/models/authorization_boundary_membership.rb` (enum → inclusion validation, `resolve_role`, `available_roles`/`acceptable_roles`), `app/models/sparc_config.rb` (`auth_boundary_roles` → `auth_boundary_role_entries`), `app/controllers/authorization_boundary_memberships_controller.rb`, `config/initializers/zz_role_config_posture.rb` | **Renamed accessor**: `SparcConfig.auth_boundary_roles` is gone — callers want `AuthorizationBoundaryMembership.available_roles` (display) or `.acceptable_roles` (validation). No migration: `role` was already a string column and the enum guaranteed every existing row holds a built-in key |
...
| [x] | **#616** Emit CycloneDX SBOM to org SCA bucket | CI/Infrastructure | `.github/workflows/sbom-and-sca.yml` (NEW — calls org-shared `container-build-sign/sbom-source.yml`+`sca-scan.yml@v0.1.0`, emits to `s3://<security-artifacts-bucket>/sca/sparc/`), `.security/sca-allowlist.yaml` (NEW), `docs/compliance/nist-sp800-53-rev5-mapping.md` (SR-3, CM-8, RA-5), `docs/compliance/oscal/cdefs/component-definition-security-scanning.json` (SR-3, RA-5 remarks), `app/models/sparc_config.rb` (VERSION → 1.8.8) | **LOW** — new workflow file + compliance docs. Non-gating (not in `required-checks.json`). Cross-repo IAM dep: `SCA_EMIT_ROLE_ARN` provisioned in sparc-iac (umbrella container-build-sign#12). Validate with `actionlint`. **Superseded twice since:** the emit target moved to the canonical `<EVIDENCE_BOUNDARY>/<date>/sparc/sca/` layout (#741, sparc-iac#537), the role in use is `SPARC_APP_CI_ARN` rather than `SCA_EMIT_ROLE_ARN`, and the boundary pivots to `risk-sentinel` with no default (#1153, sparc-iac#715). Row kept as the record of what #616 did. |
...
| [ ] | **#639** Base-image CVE audit + hardened variants | CI/Infra + Container | `Dockerfile`, `script/dev/install-hdf.sh`, `docs/compliance/sparc-findings.yml` | **MEDIUM** — image build + CVE dispositions; serialize with any base-image bump. |
...
| [x] | **#680** Durable, version-aware artifact references + version history | API (v1) + Evidence/OSCAL | **SHIPPED (v1.10.0)** — stable resolver + content-version UUID for drift; `artifact_versions` history w/ full per-version blob retention; purge default off. Touches `back_matter_resource.rb`, `evidence.rb`, `attestation.rb`, `routes.rb`, `document_conversion_job.rb` + new `artifact_version.rb`/`artifact_versionable` concern + migration. Reaper (Phase 3) open; copy-per-version #686; ODP validator #685. | **MEDIUM** — hot files `routes.rb` + `back_matter_resource.rb` + a migration; coordinate with OSCAL-export work. |
...
| [x] | **Z** ~~#1047~~ ~~#728~~ #1046 → v1.17.0 (+ ~~#1090~~ ~~#1092~~ ~~#1093~~ ~~#1094~~ ~~#1095~~ ~~#1096~~ folded, and **#1100** carried) | Shared/UI (**every screen**) + Catalog/SSP services + OSCAL export | **`app/assets/stylesheets/sparc-theme.css` — 255 → 588 classes, the single hottest file in the repo this cycle**; inline `style=` across `app/views/**` (**1,516 → 254**, 42 view files); `spec/views/inline_style_ratchet_spec.rb`; `tests/ui-smoke/visual_regression_1047.py`; 5 Stimulus controllers moved to `controllers/visibility`; `app/services/{catalog_import,catalog_part_extractor,oscal_resolved_profile_catalog,ssp_from_profile,oscal_ssp_export}_service.rb`; 2 migrations (`20260905090000`, `20260905140000`); `app/helpers/application_helper.rb` | **HIGH — `sparc-theme.css` and `app/views/**` are touched by ANY UI work.** #1047 and #728 close on this PR; the 254-declaration tail is **#1109 / v1.17.0**. #1046 not started. **#1100 stays open pending owner review.** Anyone editing a view during this window should rebase before touching `sparc-theme.css` |
...
| Status | Bundle / Issue | Domain | Files Modified | Collision Risk |
| ------ | -------------- | ------ | -------------- | -------------- |
| [ ] | **AK** **#871** | CI/Infrastructure + Compliance | `.github/workflows/deviation-approval.yml` (add `issue_comment` trigger), `scripts/ci/apply_deviation_approval.rb`, `scripts/ci/check_deviation_approvals.rb`, `spec/scripts/deviation_flow_spec.rb`, `docs/compliance/README.md` + wiki | **HIGH — this changes how an approval is granted, and a mistake grants one that was never made.** Two properties are load-bearing: the workflow AND the approval logic must execute from `main`, never the PR's copy, or a PR self-approves by editing the script; and the trigger must be `action: created` only so an edited comment cannot manufacture an approval retroactively. Needs a credential that TRIGGERS workflows (App or PAT — `GITHUB_TOKEN` pushes do not). **The `admin-merge-bypass` it retires has zero spec coverage while carrying 8 retired entries, so write its test BEFORE removing it.** Cannot be tested on its own PR — `issue_comment` runs only the default-branch workflow — so the live test is the first deviation PR after it merges. |
| [x] | **AH** ~~#1161~~ ~~#1159~~ | Federation identity / cross-repo contract | `lib/federation/` (NEW — `key-grammar.v1.json`, `key.rb`, `python/` package, `README.md`), `spec/lib/federation/key_spec.rb`, `app/services/federation_bundle_signing_service.rb` (Result gains `party`) | **LOW–MEDIUM** — `lib/federation/` is a new tree nothing else touches, and `lib/` is autoloaded so `zeitwerk:check` must pass (it does). The one shared edit is an ADDITIVE member on `FederationBundleSigningService::Result`; one production caller, 19 existing specs unaffected. **`key-grammar.v1.json` is a CROSS-REPO contract — `sparc-horizon` consumes it, and its `fixtures/` are stale until it adopts the registered namespace.** **DELIVERED 2026-09-21.** |
| [x] | **AG** ~~#1155~~ ~~#1162~~ | Federation identity + API (Evidence/SSP statements) | `app/models/oscal_namespace.rb` (FEDERATION_NAMESPACE), `app/controllers/api/v1/federation_identities_controller.rb` (NEW), `config/routes.rb`, `app/controllers/api/v1/discovery_controller.rb`, `app/controllers/api/v1/evidences_controller.rb`, `app/controllers/evidences_controller.rb`, `app/models/evidence.rb` (autosave), `app/controllers/api/v1/ssp_control_statements_controller.rb`, `docs/api/**` + Postman + generated INVENTORY, `wiki/API-Reference.md` | **MEDIUM** — `config/routes.rb` is a hot file (§5) and `docs/api/INVENTORY.md` is GENERATED (`bin/api_inventory_check.rb --write`; `--check` gates). `app/models/evidence.rb` gained `autosave: true` on `evidence_control_links`, which changes behaviour for EVERY caller of that association, not only the API — all 146 evidence specs pass with it. **DELIVERED 2026-09-21.** |
...
| `app/models/sparc_config.rb` | Every new `SPARC_*` env var lands here; v1.16.0: #914 + #909 (bundle A), #822 (bundle G), #842 (bundle I) | **MEDIUM** risk. Additive method blocks. Coordinate the section a new env var belongs in (organization metadata, rate limiting, upload caps, etc.) and add to `docs/ENVIRONMENT_VARIABLES.md` in the same PR. **v1.16.0 order: bundle A lands FIRST** — it restructures the banner and resources blocks and rewrites the corresponding sections of `docs/ENVIRONMENT_VARIABLES.md` and `wiki/Configuration.md`, which G and I then extend. |
| `spec/support/system_spec_config.rb` | #909 (v1.16.0) | **MEDIUM** and easy to miss. It force-sets banner env vars for **every** system spec, and `accept_consent_banner` depends on the banner actually rendering — so a change to banner configuration that forgets this file breaks all four system specs with an error that points at the spec, not the cause. |
| `tests/ui-smoke/a11y_baseline.json` | v1.16.0: #908 (bundle D), #880/#879 (bundle E), #904 (bundle F), #842 (bundle I) | **HIGH RISK for concurrent PRs.** A baseline regeneration rewrites the whole 1615-line file, so two open PRs touching it conflict irreconcilably. **Never have two in flight.** #880 adds chrome to every page and needs a global regeneration — sequence it after the page-scoped ones so it sweeps them up in one diff. |
...
| `app/views/shared/_dropzone.html.erb` + `app/javascript/controllers/dropzone_controller.js` | #185, #355 (multi-file), #902 (required enforcement), #947 (per-type requirement) | **MEDIUM** — 9 upload screens render this one partial, so a change here lands on every import and evidence flow at once. Note the file requirement is enforced by the Stimulus controller, NOT the native `required` attribute: the input is `d-none`, and Chrome silently refuses to submit a form with a required control it cannot focus (#902). Do not "restore" the attribute. **And do not remove the local to relax the rule (#947):** the partial reads `required = true unless defined?(required) && required == false`, so OMITTING it defaults to **true** and re-arms the controller's capture-phase `preventDefault` — the submit is then cancelled with no request made and nothing shown, which is the same silent refusal in a new place. Pass `required:` explicitly; if the requirement varies per screen state, rewrite `data-dropzone-required-value` from that screen's own controller. Caught only by a browser test. |
| `ApplicationHelper::FLASH_CLASSES` + both layouts' flash blocks | #902 | **MEDIUM** — the single source of truth for which flash keys render. A key missing from the map renders nowhere, with no error and no failing spec, which is how `notice:`/`alert:` went unseen across 12 controllers. Add new keys here, never to one layout. Covered by `spec/helpers/application_helper_spec.rb` + `spec/requests/flash_rendering_spec.rb`. |
| `app/services/collection_tiering.rb` + `app/controllers/concerns/collection_tierable.rb` + `app/views/shared/_collection_tiers.html.erb` | #948, and every future collection screen | **MEDIUM** — five index screens (evidences, SSP, SAP, SAR, POA&M) render the shared tier chrome, so a change lands on all of them at once. **The invariant is that this layer never decides visibility:** it receives an already-scoped, already-filtered relation, has no access to `current_user`, and adds no `where`. Each screen carries a spec asserting the visible set is identical before and after — if you touch this, run them all. CDEF is deliberately absent (no boundary column, #980). |
| `app/models/attestation.rb` | #440, #680 (versioning), #947 (attester verification), #981 (picker refresh) | **MEDIUM** — the attester check reads the canonical `Role`/`UserRole` system, so it is coupled to RBAC changes as well as to evidence work. Note `Attestation::ROLES` is **gone** (#947): who may attest derives from the `evidence.attest` permission. Grep for hardcoded role STRINGS before changing the vocabulary — `FindingDispositionService` gated waivers on the literal `"authorizing_official"` and broke silently when the canonical name is `ao`. |
| `app/services/oscal_resolved_profile_catalog_service.rb` + the six readers of `resolved_catalog_json` | #999 (bundle U), and every future document-from-profile generator | **HIGH RISK — this is the shared-infrastructure change the owner approved for bundle U.** The column is written by three paths (the resolver service, `ProfileJsonParserService` on import of an already-resolved profile, and the seeds) and read by **six** services that walk only `group["controls"]`: `SspFromProfileService`, `SarFromProfileService`, `CdefFromProfileService`, `CdefBaselineGapService`, `CdefBulkApplyService`, and `BaselineParameterService` (which handles one level of nesting only). Nesting enhancements — which is what a conformant resolved catalog requires — silently halves what those six see unless they go through the shared walker. **The same gap already bites today** on an imported NIST resolved catalog: the profile gets all 370 controls and a generated SSP gets 188. Add a reader, never a seventh private traversal, and run all six services' specs when you touch it. |
| `app/services/catalog_import_service.rb` + `db/seeds/` + `SeedRunner::CURRENT_VERSIONS` | #999 (bundle U), #935, #911, and every catalog-shape change | **HIGH RISK.** Control-level `links` and the catalog's back-matter resources are read here or nowhere — an exporter cannot emit what import discarded, which is why #999's link half is really an import defect. A change to what import persists is only real once the seeded catalogs are re-imported, so it **must** bump `SeedRunner::CURRENT_VERSIONS` in the same PR and ship a backfill for existing installs. Validate against the **test** database, never the owner's development one. |
| The shared control-detail partial (statement / resolved parameters / priority / guidance / related) | #997 (bundle U), #881, #942, and both document show screens | **MEDIUM** — one partial renders on the Profile screen (editable, gated `profiles.write`) and the SSP screen (read-only), so a change lands on both at once, and `control_families/show.html.erb` is the third caller it exists to avoid duplicating. **The invariant is that parameter substitution always goes through `OscalParameterResolver`:** raw `{{ insert: param, … }}` reaching the screen is the defect #942 fixed in the exporters and #997 fixes in the views. The editable/read-only split is a permission decision, not a template fork — pass it in. |

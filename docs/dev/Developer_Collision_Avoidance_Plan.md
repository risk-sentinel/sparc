...
| [ ] | **#463** SAF CLI MODULE_NOT_FOUND fix (Node 22 pin + @mitre/saf@1.6.0) | CI/Infrastructure | `.github/workflows/security.yml` (`normalize_hdf`: `actions/setup-node@v4` + pin `@mitre/saf@1.6.0`, verify step, per-conversion log capture, dump failed logs, debug `hdf-conversion-logs` artifact), `scripts/trivy-scan.sh` (header docs only) | **LOW** -- stacks on #461 branch; same security.yml region as #461 |

...
| `app/views/shared/_dropzone.html.erb` + `app/javascript/controllers/dropzone_controller.js` | #185, #355 (multi-file), #902 (required enforcement) | **MEDIUM** — 9 upload screens render this one partial, so a change here lands on every import and evidence flow at once. Note the file requirement is enforced by the Stimulus controller, NOT the native `required` attribute: the input is `d-none`, and Chrome silently refuses to submit a form with a required control it cannot focus (#902). Do not "restore" the attribute. |

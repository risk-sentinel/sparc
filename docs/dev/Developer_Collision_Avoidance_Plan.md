...
| [ ] | **#473** Aggregator: add `edited` to triggers + in-flight-aware short-circuit | CI/Infrastructure | `.github/workflows/required-passed.yml` (on.pull_request.types, poll-loop in-flight check, path-aware rule check) | **LOW** -- single workflow file |
| [ ] | **#475** License triage: review all outstanding warn/unmapped action items, record dispositions, flip enforce | Compliance/Docs | `docs/compliance/license-dispositions.yml`, `docs/compliance/license-policy.yml` (allowlist expansions, enforce flip) | **LOW** -- compliance docs only |

...
**Goal:** Close the v1.16.0 milestone (25 issues — 15 originally scoped, plus #939, #941, #942 and #936 filed during Bundle F, #944, #946, #947 + #952 found in local review of Bundle E, and #845 pulled in to make the test data real). Two structural security deliverables lead —
...
| **P0** | [ ] | #845 | Reusable two-boundary leveraged reference authorization (golden E2E fixture) | **Bundle M — NEXT, immediately after PR #943 lands.** Pulled into the milestone because our fixtures are thin enough to hide defects: measured on the #943 gate run, **18 of 32 `tests/ui-smoke` skips are data thinness, not posture** — 12 "no records on this deployment", 4 "no filter dropdown offered for this corpus", 2 "metadata not editable". **Both blockers are CLOSED** (#843 `PoamGeneratorService`, #844 `SapGeneratorService` wiring), so it can be built fully generator-backed with no interim path. Owner-specified shape: the **full chain for BOTH boundaries** — Catalog → Profile → SSP → SAP → SAR → **multiple POA&Ms** → Evidence — with the artefacts **committed to the repo as loadable test data** and a **single-command loader script**, so the suite gets a fast deterministic fixture rather than a slow end-to-end assembly. Regeneration through the productized services stays the way artefacts are refreshed, so they cannot silently drift. **DAST split out to #953.** Unblocks validation for #952, #946, #948 and #951 — and is the only thing that can *prove* #952's Evidence-leveraging exemption, which is currently asserted rather than tested. |
...
A #914 #909  ->  B #894 #897  ->  [#923 actions]  ->  C #919  ->  [#922 #921 bundler]
  ...
  ->  M #845  ->  L #944  ->  H #707  ->  I #860 #842  ->  release/v1.16.0
```
...
**Bundle M (#845) is next, immediately after PR #943 lands**, then Bundle L (#944).
Owner direction: the fixture comes first because everything after it validates against
it, and doing #952's 368-call-site factory work without a realistic estate means doing
that work twice. Both of #845's blockers (#843, #844) are closed.

**Bundle L (#944) follows** — ahead of G and I. It came out of
...
**#946 belongs with it.** Both are the same underlying failure — a document that
cannot reach the thing it is based on — and #946's fix (link `profile_document_id`
at import from the OSCAL `import-profile` href, keep the href when it does not
resolve) is the same lineage rule #911 established. Four instances now: #928
(profile → catalog), #929 (document → boundary), #944 (CDEF authoring), #946
(SSP → baseline). Worth designing the linkage rule once rather than a fourth time.
...
wait for G or I** — it leaks orphaned records on every cold pass, and the leak is
cumulative, so the cost of deferring it grows rather than staying flat. The
nav-dropdown overflow issue (raised, approved for filing, gated on a full responsive
breakpoint audit so the issue is evidence-led) is also unfiled and unsequenced.
...
| 16 | Current | v1.16.0 — config correctness, authorization sweep, UX filters, auth entitlements (milestone `v1.16.0`) | ~~#914~~, ~~#909~~, ~~#894~~, ~~#897~~, ~~#919~~, ~~#707~~, ~~#908~~, ~~#928~~, ~~#934~~, ~~#904~~, #880, #879, #929, #822, #860, #842, #939, #941, #942, #936, #944, #946, #947, #952, #845 | In Progress — **11 of 25 shipped** (PRs #924, #925, #931, #932, #933, #937, #938). **Bundle E (#880 + #879) implemented and in PR.** The issue count moved 16 → 24: #939, #941, #942 and #936 were filed during Bundle F, and #944, #946, #947 + #952 came out of local review of Bundle E. **#952 pairs with #929.** **#845 is next after PR #943 lands** — the fixture the rest validate against. **#944 is next, by owner direction.** Remaining: E in review, then G (#822 + #820), I (#860 + #842), #929, and the four Bundle F bugs — of which **#939 is the one that should not wait**. Target tag ~2026-09-21. Per-issue detail and bundle sequencing live in the Phase 16 section above; this row is the phase-level status |

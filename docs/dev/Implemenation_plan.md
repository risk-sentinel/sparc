...
| **P3** | [ ] | #447 | umbrella: SPARC as system-of-record for HDF Amendment dispositions (tenant CI/CD finding triage) | Filed 2026-05-06 alongside #440. Sibling feature: SPARC offers tenants the disposition workflow we built for our own CI (#244) — HDF ingest, finding-triage UI/API, hdf-libs HDF Amendments JSON export per system. Reuses Evidence + Attestation provenance pattern. Sequenced after #440 lands so the Attestation `frequency`/`status` fields are available. Design pass before implementation. |
...
| 12 | Current | Active Backlog — Post-migration Test/CI Hardening + Federation Follow-ups | ~~#436~~, ~~#244~~, ~~#367~~, ~~#445~~, ~~#440~~, #447, #433, #341, #246, #422, #413 | In Progress |

<!-- markdownlint-enable MD013 -->

**Total issues tracked:** 68 (23 original + 45 ad-hoc/new — adds #436, #445, #440, #447)
...
**Remaining (Phase 12 active backlog):** 6 issues — P1: #433 / P2: #341, #246 / P3: #447 (hdf-libs amendment umbrella — sibling to #440), #422 (gated on first federation deployment), #413 (umbrella; closes on #433 merge)

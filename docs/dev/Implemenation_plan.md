...
| **P1** | [ ] | #627 | API-created document marked `completed` with no required content — no gate (follow-up to #618) | In progress on `bug/627_628_document_completeness` (bundled with #628). Decouples content-completeness from the parse `status` via the `ContentCompleteness` concern; gates `Publishable#publish` on per-type required content (SSP system characteristics + ≥1 control; CDEF ≥1 control; Profile catalog + ≥1 control); surfaces an Incomplete badge in UI + `content_complete`/`content_completeness_gaps` in API serializers. |
| **P1** | [ ] | #628 | API-created CDEF/SSP is an uneditable empty shell — needs a control basis + post-create populate path | In progress on `bug/627_628_document_completeness` (bundled with #627). Adds populate-from-profile for an EXISTING empty CDEF/SSP (UI `attach_profile`/`populate_from_profile` + API `POST .../populate_from_profile`) so a metadata-only shell isn't a dead end. Profile requirement is enforced as a content-completeness prerequisite (not a hard model NOT NULL) so file-import paths and existing rows stay valid. |

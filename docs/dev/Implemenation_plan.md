...
| **P1** | [ ] | #629 | Bulk delete (multi-row select) for CDEF + Authorization Boundary index (admin) | On `feature/629_bulk_delete_cdef_auth_boundary`. `BulkDestroyService` (partial-success, per-row audit) backs both UI `bulk_destroy` and API `DELETE .../bulk`; `bulk-select` Stimulus controller. Adds an `AuthorizationBoundary` referential guard (blocks only when SSP/SAP/SAR/POA&M attached) + fixes the single-delete return-value gap. Held at v1.8.11 (bundling with next items). |

...
A #914 #909  ->  B #894 #897  ->  [#923 actions]  ->  C #919  ->  [#922 #921 bundler]
  ->  D #908 #928  ->  K #934  ->  F #904  ->  E #880 #879  ->  M #845 (+#954 #955 #956 #958)
```

**Remaining — owner-set order 2026-08-15:** OSCAL → boundary attachment → evidence →
ops/UX → auth epic.

```text
N #941 #942 #945 #946 #957 #944   ->  [#939 pulled forward]  ->  O #929 #952
  ->  P #947 #948  ->  Q #936  ->  R #860 #842 #822 (+#820 openssl)  ->  release/v1.16.0
```

**Status 2026-08-15:** A, B, C, D, K, F, E and M are **merged** (17 of 31 closed). H shipped
early, inside #919. That leaves N, O, P, Q and R.

**#939 is pulled forward, out of ops/UX** (owner, 2026-08-15). It is small, and the cost of
deferring it **grows rather than staying flat**: `write_through_parser` creates the
`CdefDocument` row before parsing, so every failed file leaks a permanent orphan that
`aws_labs_sourced` cannot see to clean up — 82 measured after four passes. They surface at the
top of the CDEF index and they break OSCAL export. Sequenced after N so it does not block the
document-model work, but before the boundary bundle.

**Bundle N is the document-model family.** #928 (profile → catalog), #929 (document →
boundary), #944 (CDEF authoring), #945 (mapping entries), #946 (SSP → baseline) are five
instances of one failure: *a record that cannot reach the thing it is based on*. #928 shipped
in Bundle D and #911 established the lineage rule. **Design the linkage rule once in N and
apply it in O** rather than a sixth time. #957 joins N because it is the same layer and the
#845 reference artifacts now give it a regression target — UUID instability shows up as drift
in a committed file.

**O after N, and #952 after #929 inside it.** #952 requires a boundary on SSP/SAP/SAR/POA&M,
but `document_metadata_params` permits only name/version/oscal_version/description, so the
boundary can be set at upload and **never after** — requiring it without #929 leaves every
orphan permanently invalid and unfixable. The cost is in the fixtures, not the validation: no
SSP/SAP/SAR/POA&M factory sets a boundary, across **368 call sites in ~149 spec files**, and
the seed itself creates boundary-less documents. #845 built the groundwork — the reference
estate attaches every document to its boundary explicitly and ships a spec pinning it, because
the generators leave it nil.

**R last, by owner direction.** This moves #820 (openssl 3.3.0 → 4.0.2) to the end of the
release, since it is paired with #822 for a single two-ceremony verification round.
`bundle-audit` reports **no vulnerabilities** against the current lock, so the deferral is
schedulable rather than reactive — but if that changes, #820 must be decoupled from #822 and
taken on its own.

**Not on the milestone, tracked elsewhere:** #953 (authenticated DAST — **unblocked** by
#845's production posture), #959 (every export embeds every authoritative back-matter
resource), #935 (derive and persist `framework` at import).

- **D after C** — both edit `profile_documents_controller`.
- **E after D and F** — the offcanvas forces a global `a11y_baseline.json` regeneration.

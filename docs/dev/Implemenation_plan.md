...
| **P1** | [x] | #680 | ~~Durable UUID-addressed artifact resolver (Phase 1)~~ — **SHIPPING (v1.10.0)** | `/artifacts/:uuid` (web + `Api::V1`) → freshly-signed blob URL; OSCAL back-matter hrefs switched to the immutable resolver URL (survives rename/re-upload/expiry). Phases 2–3 (retain-on-reference, orphan reaper) remain open. NIST AU-10/SI-12. |

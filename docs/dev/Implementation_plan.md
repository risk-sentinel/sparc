...
| **CI-3 — Test-job fidelity** | #835 #927 #711 **#1061** | The HDF translation specs do not actually run without a pinned `hdf-cli` in the test job (#835); deprecation warnings flood the log (#927); and there is no deployed API-contract runner (#711). **#1061** joins them: publishing the wiki was a manual step, so merging `wiki/` changed nothing a reader could see — the same shape as the rest of this bundle, a step that looks done and is not. **#1061's stated blocker was wrong** — it assumed this needed a PAT or GitHub App, but a GitHub App cannot push to a wiki at all (no such permission exists) and the built-in `GITHUB_TOKEN` with `contents: write` can. No credential provisioned. | 2d |

...
## Extending to SAR updates

### app/services/sar_update_service.rb

```ruby
class SarUpdateService
  ...
  def update_control(control_id, field_updates)
    ...
    field_updates.each do |field_name, new_value|
      field = control.sar_control_fields.find_or_initialize_by(field_name: field_name)
  ...
  def update_test_status(control_id, status, results)
    ...
    status_field = control.sar_control_fields.find_or_create_by!(field_name: 'test_status')
    status_field.update!(field_value: status)

    results_field = control.sar_control_fields.find_or_create_by!(field_name: 'test_results')
    results_field.update!(field_value: results)

    date_field = control.sar_control_fields.find_or_create_by!(field_name: 'test_date')
...
### Add API endpoint for SAR updates

```ruby
# app/controllers/api/v1/sar_documents_controller.rb
module Api
  module V1
    class SarDocumentsController < ApplicationController
      ...
      def update_fields
        ...
        update_service = SarUpdateService.new(sar_document)

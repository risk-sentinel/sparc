...
- **Code:** **211 logical endpoints** across 43 controller groups (PATCH+PUT aliases collapsed) — 81 GET, 75 POST, 28 DELETE, 25 PUT, 23 PATCH
- **Documentation:** **168 / 211** endpoints are listed in an `endpoints/*.md` page's own Endpoints table (**80%**); 24 more are mentioned only in prose
- **Postman collection:** **211 / 211** endpoints covered (**100%**)
- **Pytest suite:** **173 / 211** endpoints map to a `tests/api/test_*.py` module (**82%**)

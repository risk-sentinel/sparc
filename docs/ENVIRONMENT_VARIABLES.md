...
| SPARC_REQUIRE_AUTH_METHODS | CSV allowlist of the login methods allowed to hold a session; empty = no restriction. A signed-in user whose method isn't listed is bounced to `/login`. Tokens: `local`, `ldap`, `piv`, `webauthn`/`fido2`, `oidc`, `github`, `gitlab`, `sso`. Break-glass admin + service accounts exempt. App-layer, so it works on single-listener `optional` mTLS. (#805) | (empty) | `oidc,piv` | No |

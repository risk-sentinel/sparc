...
| SPARC_REQUIRE_AUTH_METHODS | CSV allowlist of the login methods allowed to hold a session; empty = no restriction. A signed-in user whose method isn't listed is bounced to `/login`. Tokens: `local`, `ldap`, `piv`, `webauthn`/`fido2`, `oidc`, `github`, `gitlab`, `sso`. Break-glass admin + service accounts exempt. App-layer, so it works on single-listener `optional` mTLS. (#805) **Since v1.16.1 (#1082) this variable also ROUTES ENABLEMENT** — requiring `local`, `ldap`, `piv` or `fido2` enables it, so you set one variable rather than two; an explicit `SPARC_ENABLE_*=false` still wins. It cannot supply a credential, so `oidc`/`github`/`gitlab` still need their client id and `ldap` still needs a host. **A production instance whose required methods are ALL unusable now fails at boot** instead of locking every account out at request time. | (empty) | `oidc,piv` | No |
...
| SPARC_PIV_LOGIN_REQUIRES_CERT | When `true`, the login page shows the PIV/CAC button **only if** the gateway forwarded a verified client cert (`SPARC_PIV_VERIFY_HEADER` == success) on the login request. Default `false` keeps the always-show button (when PIV is enabled) so it can't vanish before the gateway forwards the header. Note: the cert is presented at the TLS handshake, so a cert inserted *after* page load won't reveal the button until a reload/new connection; this hides the button only, not the browser's cert prompt. (#808) | false | `true` | No |
| SPARC_PIV_IDENTITY_SOURCE | Which certificate field provides the primary identifier matched against a provisioned PIV identity (#790): `edipi_cn` (DoD EDIPI, last dotted CN segment), `upn` (PIV UPN in the SAN otherName), `email` (rfc822Name SAN), or `subject_cn` (whole CN). Default is DoD behaviour. See [PIV_IDENTITY_MAPPING.md](PIV_IDENTITY_MAPPING.md). | edipi_cn | `upn` | No |
| SPARC_PIV_UID_PATTERN | Optional regex applied to the identity-source value to extract the identifier — for non-DoD PKI whose uid is not a 10-digit EDIPI. First capture group wins, else the whole match. Unset ⇒ `edipi_cn` validates a 10-digit segment; other sources use the raw value. (#790) | (none) | `employee-(\d+)` | No |
| SPARC_PIV_ALLOW_EMAIL_MATCH | When `true` (default) a cert with no matching PIV identity falls back to matching its rfc822Name against `User.email`. Set `false` for high-assurance deployments to require an explicit PIV-identity mapping and refuse email matches — the email path is only as strong as the proxy's PKI trust boundary. (#790) | true | `false` | No |
| SPARC_PIV_ACCEPTED_ISSUERS | Optional defense-in-depth (#804): CSV of issuer DNs (case-insensitive substring match) the forwarded cert must come from. Empty = accept any cert the gateway validated. The gateway remains the primary CA trust. | (empty) | `ACME Corp PIV CA` | No |
| SPARC_PIV_ACCEPTED_POLICY_OIDS | Optional defense-in-depth (#804): CSV of certificate-policy OIDs the forwarded cert must carry. Empty = no policy check. | (empty) | `2.16.840.1.101.3.2.1.3.13` | No |
...
| SPARC_OSCAL_NS | Namespace for OSCAL properties **this deployment defines** — locally-coined props and organization-defined roles. Props under it are never conformance violations: SPARC validates claims about other authorities' vocabularies, not the operator's own (#1106). An OSCAL `ns` identifies WHOSE vocabulary a name belongs to, so set it to a URI your organization owns | `https://sparc.risk-sentinel.org/ns` | `https://oscal.att.com/ns` | No |
| SPARC_OSCAL_ORG_NAME | Organization named as the responsible party in every exported OSCAL artifact, and as the source of locally-authored CDEFs. SPARC is the tool, not the author — leave it unset and your SSP names "SPARC Export" as accountable for it (#1106) | `SPARC Export` | `AT&T Policy Department` | No |

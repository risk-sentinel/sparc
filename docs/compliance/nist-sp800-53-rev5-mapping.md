...
| AU-2 | Event Logging | H | Application (SPARC) | 139 audit event types across 16 categories covering authentication, authorization, user management, document lifecycle, evidence, and more. API CRUD operations on SSP, SAR, SAP, POA&M documents generate audit events for all mutations | `app/models/audit_event.rb`, `app/controllers/api/v1/base_controller.rb` | Implemented |
| AU-3 | Content of Audit Records | H | Application (SPARC) | Each audit event records: user, action, provider, IP address, user agent, structured metadata (JSONB), subject type/ID, timestamp | `app/models/audit_event.rb` | Implemented |
...
| AU-12 | Audit Record Generation | H | Application (SPARC) | `AuditEvent.log` factory called throughout controllers and models; `audit_log` helper in API base controller generates records for all document CRUD mutations | `app/models/audit_event.rb`, `app/controllers/api/v1/base_controller.rb` | Implemented |

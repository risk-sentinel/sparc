...
| RA-5 | Vulnerability Monitoring and Scanning | H | Hybrid | 9 automated security scanners: Gitleaks, Brakeman, CodeQL, Semgrep, bundler-audit, importmap audit, Trivy FS, Trivy Container, CycloneDX. Container vulnerability baseline tracked in `docs/compliance/sparc-findings.yml` with 76 CVE dispositions, 30-day review cycle, and deployment-agnostic mitigating controls | `.github/workflows/security.yml`, `docs/compliance/sparc-findings.yml` | Implemented |
...
| SI-2 | Flaw Remediation | H | Hybrid | bundler-audit and Trivy detect known vulnerabilities; dependency updates via Gemfile; container rebuilds. Accepted findings documented in `sparc-findings.yml` with rationale, NIST control mapping, and 30-day review cycle. Fixable findings remediated (rexml 3.4.4) | `.github/workflows/security.yml`, `Gemfile`, `docs/compliance/sparc-findings.yml` | Implemented |

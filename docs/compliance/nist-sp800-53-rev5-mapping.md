...
| SC-8 | Transmission Confidentiality and Integrity | H | Hybrid | `force_ssl` with HSTS (1 year, subdomains, preload); LDAP uses simple_tls or start_tls with server-certificate verification (`VERIFY_PEER`, `#773`) — the directory server's identity is authenticated, not just the channel encrypted | `config/environments/production.rb`, `app/services/ldap_auth_service.rb` | Implemented |

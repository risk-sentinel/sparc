...
| IA-2(1) | Multi-Factor Authentication to Privileged Accounts | H | Application (SPARC) | **App-native MFA** via FIDO2/WebAuthn (`SPARC_FIDO2_ENABLED`, `#779`): a security key + PIN is possession + knowledge in one passwordless ceremony (`userVerification:required`), no external IdP needed. Alternatively OIDC with `SPARC_OIDC_FORCE_MFA=true` delegates MFA to the IdP. Not met with local-password-only login (the break-glass admin path) | `app/controllers/webauthn_sessions_controller.rb`, `app/models/sparc_config.rb` (`SPARC_FIDO2_ENABLED`, `SPARC_OIDC_FORCE_MFA`) | Implemented |
| IA-2(2) | Multi-Factor Authentication to Non-Privileged Accounts | H | Application (SPARC) | Same app-native FIDO2/WebAuthn MFA as IA-2(1) (`#779`), or OIDC IdP-enforced MFA. Not met with local-password-only login | `app/controllers/webauthn_sessions_controller.rb`, `SPARC_FIDO2_ENABLED` | Implemented |

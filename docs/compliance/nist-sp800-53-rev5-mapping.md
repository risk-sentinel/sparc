...
| CA-7 | Continuous Monitoring | H | Hybrid | Weekly scheduled security scans; HDF normalization enables dashboard integration; audit event stream. KSI validations support scheduled re-validation with next_validation_due tracking and auto-expiration | `.github/workflows/security.yml`, `app/models/audit_event.rb`, `app/models/ksi_validation.rb`, `app/services/ksi_export_service.rb` | Implemented |

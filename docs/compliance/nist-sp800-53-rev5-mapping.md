...
| IA-2(12) | Acceptance of PIV Credentials | H | Hybrid | **Native PIV/CAC acceptance** (`SPARC_ENABLE_PIV`, `#779`): SPARC consumes a client certificate validated by the mTLS gateway (DoD PKI chain + revocation at the proxy/ALB, `sparc-iac#559`), parses the EDIPI/email, and maps it to a user — fail-closed unless the gateway attests verification. Also accepts PIV via OIDC when the IdP is so configured | `app/controllers/piv_sessions_controller.rb`, `app/services/piv_auth_service.rb`, `SPARC_ENABLE_PIV`, sparc-iac | Implemented |

...
| AC-2(3) | Disable Accounts | H | Application (SPARC) | `deactivate!` and `reactivate!` methods; `SPARC_INACTIVITY_DAYS` configurable threshold, applied by `InactivityCheckJob` via the `User.inactive_past_threshold` scope. Two availability exemptions (#878), both enforced in the model so every path inherits them: (1) the break-glass admin (`SparcConfig.admin_email`, matched case-insensitively) is exempt from inactivity deactivation and password expiry, since that credential is rotated out of band; the exemption is the account, not the `admin` bit — named admins still age out. (2) Deactivating **or suspending** the last active administrator is refused (`User::LastAdminError` plus a status-change validation) and audited as `user_deactivate_refused` / `user_suspend_refused`. Rationale: authentication gates on `active?` and reactivation requires another admin, so disabling the only administrator denies administration with no self-service recovery — AC-2(3) must not defeat AC-2 | `app/models/user.rb`, `app/jobs/inactivity_check_job.rb`, `app/controllers/admin/users_controller.rb` | Implemented |
...
| AC-3 | Access Enforcement | H | Application (SPARC) | RBAC with 29 roles and **35** granular permission keys via JSONB; boundary-scoped authorization. **Controller authorization sweep (#919)** — 14 controllers exposed mutating actions with no authorization of any kind: the seven POA&M children (findings, items, local components, milestones, observations, remediations, risks), `attestations`, `back_matter_resources`, `boundaries`, `control_back_matter_links`, `profile_controls`, `profile_documents`, and `Api::V1::BaselineParametersController`. Nesting did not scope them — each loaded its parent with an unscoped `find_by!(slug:)`, so knowing a slug was sufficient to rewrite POA&M findings, remediation dates, profile baselines, back-matter provenance and attestations. All are now gated with `authorize_permission!` scoped to the owning boundary, mirroring the `Api::V1` sibling for each resource so the two surfaces cannot diverge. Guard coverage is enforced structurally: `spec/security/controller_authorization_coverage_spec.rb` enumerates every routed controller with a mutating action and fails when one is neither guarded nor justifiably allowlisted, so a new unguarded controller fails on arrival. API enforces boundary-scoped RBAC on all document CRUD endpoints (SSP, SAR, SAP, POA&M) via `DocumentBaseController`. Phase 2 API adds 4 controllers (catalogs, profiles, CDEFs, control mappings) that enforce Bearer token auth; catalogs and mappings require admin role for write operations. `BaselineParametersController` enforces Bearer auth on parameter CRUD endpoints. `KsiCatalogController` (read-only) and `KsiValidationsController` (CRUD) enforce Bearer token auth on all KSI endpoints. Discovery endpoint (`GET /api/v1/available`) enforces authorization scoping — callers only see endpoints/methods they are permitted to use. Service account tokens enforce endpoint-scoped access via `allowed_endpoints` JSONB attribute and CIDR-based IP restrictions. The Controls layer (catalogs, baselines, mappings) read actions are gated behind authentication by default via `require_authentication_unless_public_controls` (`app/controllers/concerns/authentication.rb`); a deployment opts into public read + nav visibility with `SPARC_PUBLIC_CATALOGS=true` (#726). The document *web* controllers (Evidence, SSP, SAR, SAP, POA&M) enforce the same boundary-scoped access as the API via the `BoundaryScopedDocument` concern — index scoped to the user's boundaries + global (nil-boundary) records, per-record read/write authorization keyed off `<type>.read`/`<type>.write`; CDEF (global, no boundary) gates writes on `cdef.write` (#738). #770 unifies the boundary Personnel Roster so access records from BOTH `user_roles` (canonical) and legacy `authorization_boundary_memberships` are shown (`AuthorizationBoundary#personnel_roster`) — previously the boundary screen displayed only one path; and adds the boundary→organization assignment matrix (`BoundaryOrganizationAssigner`): moving a boundary between organizations is instance-admin-only, attaching an *unassigned* boundary requires org_admin on the target org. #875 makes the legacy boundary-membership role vocabulary configurable in fact rather than in name: `role` was an enum pinned to the seven built-ins, so `SPARC_AUTH_BOUNDARY_ROLES` could only subset them and any other value raised at assignment. It is now an inclusion validation over `AuthorizationBoundaryMembership.acceptable_roles` (the built-ins **plus** whatever is configured, so narrowing the dropdown can never invalidate an existing assignment), with configured entries resolved to canonical values — form normalized mechanically, vocabulary mapped through an explicit alias table — so `ISSO`/`isso` are one role rather than two look-alike grants. The check is scoped to `role_changed?`, so retiring a role from the configuration leaves existing members intact and editable instead of making every subsequent save of those records fail. #845 adds a cross-organization enforcement proof over a real leveraged authorization: a boundary that *leverages* another inherits declared implementation prose and nothing else, and cannot reach the provider's SSP, SAP, SAR or POA&Ms by index or by direct URL, on either the controller or the API path. The spec asserts in both directions — the provider's own member reads every document the consumer is refused — so the isolation cannot be an artefact of an empty fixture, and it is mutation-checked against removal of the index scope, the read guard and the API scope **#929/#952 — boundary re-association and the nil-boundary fallback.** `BoundaryScopedDocument#authorize_document_write!` and `Api::V1::DocumentBaseController#authorize_document_write!` both computed `record&.authorization_boundary_id || params[...]`, so once a record held a boundary the boundary being *requested* was never authorized — `*.write` on boundary A was sufficient to move a document into boundary B on either surface. Both now require the permission on the boundary a document LEAVES and the one it JOINS, and the top-level `authorization_boundary_id` parameter is honoured only for the attach action, because that same parameter name is the index filter and a query string must never select which boundary is checked. The attach path itself is a single shared concern (`BoundaryAttachable`) serving both the document screen and the boundary's Artifact Summary, so the two entry points cannot diverge; every attach emits a `<type>_document_boundary_attached` audit event recording both the previous and the new boundary. Which boundaries a form may OFFER now derives from the same rule the scope uses (`BoundaryPickerHelper#assignable_boundaries`): the pickers previously joined the legacy roster on `authorization_boundary_memberships.user_id`, a nullable column, while permissions run off `UserRole` — measured on a demo instance, both paths returned zero for every user including Instance-Admin, so the field was omitted from the page entirely and no uploaded document could be associated at all **#974 — the Controls layer is gated by ONE declaration, and a test fails when it is missing.** `skip_before_action :require_authentication` makes an action unconditionally public; the Controls layer pairs it with `require_authentication_unless_public_controls` so the action is public only when `SPARC_PUBLIC_CATALOGS=true`. Two lines that had to agree, repeated per controller — and `CdefDocumentsController` and `ControlFamiliesController` had the skip and not the gate, so the component-definition library and every control-family page were readable by anyone, in every posture, with no setting that turned it off (measured unauthenticated on a prod-mode instance: 200 where every sibling returned 302). `Authentication.public_controls_read only: [...]` now writes both halves from one declaration and is read-only by construction — passing a mutating action raises, so it cannot become the route by which a write is made public. Enforcement is structural: `spec/security/public_read_coverage_spec.rb` enumerates every controller that skips authentication and fails when one lacks the conditional gate or a justified allowlist entry, and `spec/requests/public_controls_access_974_spec.rb` plus `tests/ui-smoke/test_public_controls_974.py` assert the behaviour in BOTH postures against stubs and a real deployment respectively — the browser file refuses to run unless the posture it is handed matches the deployment it finds, so it can never pass by silently skipping. The flag governs the WEB UI only: `/api/v1` requires a Bearer token in every posture, and boundary documents (SSP/SAP/SAR/POA&M/Evidence) are never public in either **#947 — the authority to ASSERT is not the permission to write.** An attestation records who vouched for a piece of evidence, and `attester_name` / `attester_email` were free strings with no reference to an account and no tie to a boundary, while the role claimed came from a hardcoded list (`control_owner system_owner isso ciso assessor authorizing_official`) that matched NEITHER role system — `control_owner` existed in neither the 19 canonical boundary `Role` names nor the 7 membership roles, so an attestation could name a role nobody in the product could hold and no check could ever pass. Anyone holding `evidence.write` could therefore record "System Owner" against any name. Who may attest is now expressed as a permission (`evidence.attest`), the attester resolves to a `User`, and the claimed role is checked against what that account actually holds ON THE EVIDENCE'S BOUNDARY via `UserRole`. The check is deliberately boundary-scoped: `has_permission?` matches `authorization_boundary_id: [id, nil]`, so an instance-scoped grant would satisfy it on every boundary at once, and `Attestation.attestable_roles` therefore drops to boundary-scoped roles whenever a boundary is present. Instance-wide evidence — provider material inherited from a leveraged SSP, which belongs to no single system — admits instance-scoped attesting roles instead, so Policy may sign for shared evidence without gaining authority over any individual system's. Instance Admins clear the check the way they clear every other permission, but only under a role that carries the permission: the bypass is on WHO HOLDS the role, never on what the vocabulary contains | `spec/requests/reference_estate_isolation_spec.rb`, `app/controllers/concerns/boundary_scoped_document.rb`, `app/controllers/concerns/boundary_attachable.rb`, `app/helpers/boundary_picker_helper.rb`, `spec/requests/boundary_attachment_929_spec.rb`, `app/controllers/concerns/authentication.rb`, `spec/security/public_read_coverage_spec.rb`, `spec/requests/public_controls_access_974_spec.rb`, `tests/ui-smoke/test_public_controls_974.py`, `spec/requests/api/v1/boundary_reassociation_929_spec.rb`, `app/services/boundary_organization_assigner.rb`, `app/models/authorization_boundary.rb`, `app/models/authorization_boundary_membership.rb`, `app/controllers/concerns/authorization.rb`, `app/controllers/api/v1/document_base_controller.rb`, `app/controllers/api/v1/control_catalogs_controller.rb`, `app/controllers/api/v1/profile_documents_controller.rb`, `app/controllers/api/v1/cdef_documents_controller.rb`, `app/controllers/api/v1/control_mappings_controller.rb`, `app/controllers/api/v1/baseline_parameters_controller.rb`, `app/controllers/api/v1/ksi_catalog_controller.rb`, `app/controllers/api/v1/ksi_validations_controller.rb`, `app/controllers/api/v1/discovery_controller.rb`, `app/models/api_token.rb`, `app/controllers/admin/service_accounts_controller.rb` | Implemented |
...
| AC-4 | Information Flow Enforcement | H | Hybrid | Authorization boundaries scope data access; network-level controls in sparc-iac. Outbound HTTP(S) is centralized through `SparcHttp` so all external egress (OIDC, federation, content refreshers) routes through a mandated egress proxy when `HTTPS_PROXY`/`HTTP_PROXY` is set — honored scheme-strictly with `NO_PROXY` bypass (`#775`) | `app/controllers/concerns/authorization.rb`, `app/lib/sparc_http.rb`, sparc-iac | Implemented |
| AC-5 | Separation of Duties | H | Application (SPARC) | 29 distinct roles across 10 instance-level and 19 boundary-scoped roles; granular permission keys prevent privilege concentration. **Promotion is split across tiers (#919)**: a boundary holds `back_matter.promote` (request) while the instance policy tier holds `back_matter.approve_promotion` and `back_matter.federate` (adjudicate), so no role can approve its own promotion and the review queue cannot be bypassed. **Assessors are excluded from back-matter authoring** — `assessor_3pao` holds `sap.write`/`sar.write` but not `back_matter.*`, so an assessor cannot edit the provenance it is assessing. Both properties are pinned by `spec/security/seeded_permission_grants_spec.rb`, which fails if a seed edit grants either approval leg to the boundary tier | `app/models/user.rb`, `app/models/role.rb`, `db/seeds.rb`, `app/services/back_matter_resource_promotion_service.rb` | Implemented |
| AC-6 | Least Privilege | H | Application (SPARC) | Default deny; `authorize_permission!` checks granular permission keys; admin bypass is explicit boolean, not a role. **Permission vocabulary reconciled (#919)** — 13 of 35 keys (37%) were anomalous: 11 were enforced by code but granted to no role, which (because `User#has_permission?` returns true for admins) silently made back-matter authoring, promotion and catalog/profile/CDEF approval instance-admin-only rather than least-privileged; `converters.read` was granted to 7 roles and enforced nowhere, advertising a boundary that did not exist; and `authorization_boundaries.manage_members` was documented, granted to nobody and enforced nowhere. The seeds now grant boundary-scoped back-matter to the 14 boundary roles that already write documents (excluding `assessor_3pao`, `view_only`, and the three oversight roles that write no document), the instance tier to `policy_manager`, and roster management to ISSM/ISSO/SO-ISO. `converters.read` was removed; `admin.rotate_credentials` remains admin-only by design. Drift is prevented by `spec/security/permission_vocabulary_spec.rb`, which fails when a key is defined but unenforced, defined but undocumented, or documented but undefined. API non-admin users see only documents within their assigned authorization boundaries. Phase 2 API enforces admin-only gates for catalog and control-mapping writes; profile and CDEF endpoints require authenticated user (any role). Discovery endpoint (`GET /api/v1/available`) omits write methods for read-only callers and hides admin-only endpoints (users, catalog writes, mapping writes) from non-admins. Service accounts cannot be assigned admin role; endpoint-scoped access (`allowed_endpoints`) and CIDR restrictions enforce least privilege on service account tokens. #770 org-scoped least privilege: since SPARC has no org-scoped permission catalog, the `org_admin` OrganizationMembership role is the lever (`User#org_admin_of?`) — a non-admin org_admin may only attach an *unassigned* boundary into their own org, and moving a boundary between organizations is instance-admin-only (`BoundaryOrganizationAssigner`) **#952 — a per-system document is no longer instance-wide by omission.** `boundary_scoped_relation` matched `boundary_ids + [nil]` and the per-record read guard returned early on a nil boundary ("global -> open to all"), so an SSP, SAP, SAR or POA&M that reached the database without a boundary was readable by EVERY authenticated user, by index and by direct URL. Measured on a demo instance: 1 of 2 SSPs and 2 of 2 SARs, produced by the seed itself. Those four types now require a boundary (`validates :authorization_boundary, presence: true`) and opt out of the fallback via `boundary_scoped ..., global_fallback: false`, so a record predating the rule is visible to Instance-Admins only until it is repaired through the #929 attach flow; a deferred migration reports the strays and deliberately assigns none, since choosing a boundary for an SSP would assert an authorization relationship nobody made. **Evidence keeps the fallback by design** — it is leveraged and inherited across boundaries, so a boundary-less evidence record is a legitimate instance-wide artifact, and a spec pins that exemption with its rationale. CDEFs are out of scope: a component definition states that a control CAN be satisfied rather than how it is implemented for one system, and carries no boundary column **#947 — `evidence.attest`, and what it is deliberately NOT granted to.** Seeded to the seven accountable boundary roles (`ao`, `agency_ao`, `so_iso`, `issm`, `isso`, `ciso`, `common_control_provider`) plus `policy_manager` at the instance tier for provider/instance-wide evidence. **`assessor_3pao` is excluded on separation of duties** — an assessor must not attest to the evidence it will later assess, the same reasoning that excluded it from back-matter authoring in #919. The permission is also STRIPPED from `PERM_SAO`, which is built as `PERM_CISO.dup`: because an instance-scoped grant satisfies a boundary-scoped check on every boundary, inheriting it would have handed one oversight role the authority to sign for every system on the instance — estate-wide attestation authority arriving silently through a copy. Granting it to no role was rejected explicitly: that is the #919 anti-pattern in which 11 keys were enforced by code and granted to nobody | `app/controllers/concerns/authorization.rb`, `app/controllers/api/v1/document_base_controller.rb`, `app/controllers/api/v1/control_catalogs_controller.rb`, `app/controllers/api/v1/control_mappings_controller.rb`, `app/controllers/api/v1/discovery_controller.rb`, `app/controllers/admin/service_accounts_controller.rb`, `app/models/api_token.rb`, `app/services/boundary_organization_assigner.rb`, `app/models/user.rb`, `app/controllers/concerns/boundary_scoped_document.rb`, `app/models/ssp_document.rb`, `spec/models/boundary_required_952_spec.rb`, `spec/requests/ssp_documents_authorization_spec.rb`, `db/migrate/20260817140000_report_boundary_less_documents.rb` | Implemented |
...
| AC-12 | Session Termination | H | Application (SPARC) | `end_session` calls `reset_session` to clear all data; explicit logout; **idle** termination (`SPARC_SESSION_TIMEOUT_MINUTES`, default 60) and **absolute** termination (`SPARC_SESSION_MAX_HOURS`, default 8) measured from sign-in and never reset by activity, so a continuously active session is still bounded | `app/controllers/concerns/authentication.rb` | Implemented |
...
| AU-10 | Non-Repudiation | H | Application (SPARC) | Audit events link actions to authenticated user_id, IP address, and user agent; immutable records. **Durable artifact references (#680 Phase 1)** — exported OSCAL back-matter hrefs use an immutable artifact UUID resolved via `/artifacts/:uuid`, so a reference cannot be silently re-pointed by evidence rename or re-upload; the cited artifact's identity is stable and verifiable. **Version-aware identity (#680)** — the back-matter `resource.uuid` is a content-version UUID (a fingerprint over the file + attestations + evidence status), so a stable resolver link paired with a changing UUID makes artifact drift detectable across documents; a permanent `artifact_versions` history (uuid, reviewed/as-of date, attester snapshot, retained per-version content) is the immutable audit trail. **Signed HDF package (#809)** — the amendments + findings + dispositions bundle a consumer receives is HMAC-SHA256 signed with an instance key derived from `SPARC_HASH`, and each disposition's `signature_hash` binds both its decider and its approver **Evidence collection provenance (#738/#903)** — `collected_at` / `collected_by` are server-stamped on every write path (web and API) and are not permitted request parameters, so the collector cannot self-assert who gathered an artifact or when. As of #903 the form no longer presents them as editable fields (they were accepted, then silently discarded, which misrepresented the record to the submitter), and a model validation rejects a future `collected_at` for any path that can set it. **Provenance on every creation path, bound to the account (#934)** — stamping moved into `Evidence#stamp_collection!`, the single writer all three creation paths call, because the third one (`AuthoritativeSourceFetchService`) recorded no collector at all while its caller already knew the actor; auto-fetched artifacts entered the corpus unattributed. A `collected_by_user_id` reference is now recorded alongside the `collected_by` name, so an artifact is attributable to an account rather than to a string that two people can share and a rename can orphan — including evidence submitted by a service-account token, which is attributed to the service account itself. `collected_by` is retained unmodified as the snapshot of what was true at collection time and is never derived from the reference; the backfill leaves an ambiguous or unmatched name unattributed rather than guessing **Attester provenance bound to an account, with a snapshot (#947)** — an attestation is the record whose whole substance is WHO asserted it, so `attester_user_id` now references the account and the claimed role is verified against the boundary roster at write time. Following the #934 rule, `attester_name` is SNAPSHOTTED from the account when the attester is set and never derived on read, so a later rename, role change or deactivation cannot rewrite what the record said at the moment it was signed; `signature_hash` is untouched by any of this. Rows written before the rule cannot be resolved automatically — an `attester_name` string is not reliably a person, and matching one to an account by name would put a real, checkable identity behind an assertion that was never checked — so they are REPORTED by a deferred advisory migration and remain readable, while the validation requires a resolved attester on the next write. The same migration reports evidence that supports no control and artefact-type evidence carrying no file; it changes nothing, because choosing a control for a piece of evidence would assert a claim nobody made | `app/models/audit_event.rb`, `app/services/hdf_package_service.rb`, `app/controllers/artifacts_controller.rb`, `app/controllers/api/v1/artifacts_controller.rb`, `app/controllers/concerns/artifact_resolvable.rb`, `app/models/evidence.rb`, `app/services/authoritative_source_fetch_service.rb`, `app/models/artifact_version.rb`, `app/models/concerns/artifact_versionable.rb` | Implemented |
...
| AU-12 | Audit Record Generation | H | Application (SPARC) | `AuditEvent.log` factory called throughout controllers and models; `audit_log` helper in API base controller generates records for all document CRUD mutations. Phase 2 API extends `audit_log()` coverage to catalog, profile, CDEF, and control-mapping controllers for create/update/destroy actions. Baseline parameter updates are audit-logged via `audit_log()` in `BaselineParametersController`. KSI validation mutations (create, update, delete) are audit-logged via `audit_log()` in `KsiValidationsController`. Admin-only bulk delete (#629, CDEF + Authorization Boundary) audits **every** row — `<model>_deleted` on success and `<model>_delete_blocked` when a referential-integrity guard prevents it — via `BulkDestroyService`. **Scan history as audit record (#811)** — re-ingesting a control does not overwrite its prior finding: the previous row is demoted to `current=false` / `superseded` and retained, so every scan run and every lifecycle transition (new / carried_forward / re_failed / expired) stays reconstructable | `app/models/audit_event.rb`, `app/controllers/api/v1/base_controller.rb`, `app/services/hdf_ingest_service.rb`, `app/models/scanner_finding.rb`, `app/controllers/api/v1/baseline_parameters_controller.rb`, `app/controllers/api/v1/ksi_validations_controller.rb`, `app/services/bulk_destroy_service.rb` | Implemented |
...
| CM-6 | Configuration Settings | H | Hybrid | `SparcConfig` centralizes all SPARC_* env var settings with secure defaults; infrastructure config in sparc-iac. `SPARC_API_OIDC_AUDIENCE` for JWT audience validation. `SPARC_API_AUTH` env var selects mutually exclusive API auth mode (`local`, `oidc`, `hybrid`); boot-time validation in `config/initializers/api_auth.rb` ensures correct configuration at startup (e.g., OIDC settings present when mode requires them). `BaselineParameterService` enables API-driven baseline parameter customization for profile documents. `SPARC_AWS_SECRETS_ENABLED` and `SPARC_APP_CONFIG_SECRET_ARN` control AWS Secrets Manager integration for injecting secrets from a KMS-encrypted JSON blob into ENV at boot. `SPARC_AWS_IAM_DB_AUTH` enables IAM database authentication with auto-rotating 15-minute tokens. Container image baseline documented in `sparc-findings.yml` -- hardened Dockerfile (non-root, multi-stage, minimal runtime deps). Configuration that defines an access-control vocabulary is resolved and reported rather than trusted verbatim: `SPARC_AUTH_BOUNDARY_ROLES` entries are normalized and resolved against the built-in roles, and anything left over is accepted as a custom role and logged at boot as such by `zz_role_config_posture.rb` — reported, never fatal, so an operator's role-list typo is visible in the start-up log without being able to stop the system from starting (#875) | `app/models/sparc_config.rb`, `app/models/authorization_boundary_membership.rb`, `app/services/baseline_parameter_service.rb`, `app/controllers/api/v1/baseline_parameters_controller.rb`, `config/initializers/api_auth.rb`, `config/initializers/zz_role_config_posture.rb`, `config/initializers/00_aws_secrets.rb`, `config/initializers/aws_db_auth.rb`, `Dockerfile`, `docs/compliance/sparc-findings.yml` | Implemented |
...
| CM-8 | System Component Inventory | H | Hybrid | CycloneDX SBOM generation for Ruby dependencies; Trivy CycloneDX for container components. **AWS-sourced CDEF provenance (#466)** — every runtime-imported CDEF row carries `source_url`, `source_sha`, `source_commit_sha`, and `fetched_at` so the in-app catalog is auditable back to the upstream blob. **Org SCA emission (#616)** — the source CycloneDX inventory is published to `s3://<security-artifacts-bucket>/<EVIDENCE_BOUNDARY>/{<date>,latest}/sparc/sca/sparc.cdx.json` on every main push + daily, contributing sparc's component inventory to the org-wide aggregated view. **Infrastructure component inventory from IaC (#904)** — the CDEF coverage wizard derives a boundary's deployed service inventory from its Terraform state or plan and reports which Component Definitions the boundary needs (adopt upstream / keep custom / author / retire). The inventory is derived from resource `mode`, `type` and instance count only; the uploaded state is parsed in-request and never stored, so the inventory exists without the secrets Terraform state carries. Unrecognised resource types are reported as coverage gaps under an inferred, namespaced service key rather than omitted, so a boundary on an unmapped provider cannot read as fully covered. | `.github/workflows/security.yml` (sbom_generation job), `.github/workflows/sbom-and-sca.yml`, `app/services/aws_labs_cdef_import_service.rb`, `app/services/cdef_coverage_analysis.rb`, `app/services/terraform_upload_inventory_service.rb` | Implemented |
...
| IA-2(1) | Multi-Factor Authentication to Privileged Accounts | H | Application (SPARC) | **App-native MFA** via FIDO2/WebAuthn (`SPARC_FIDO2_ENABLED`, `#779`): a security key + PIN is possession + knowledge in one passwordless ceremony (`userVerification:required`), no external IdP needed. **Enforced** — not merely available — via `SPARC_REQUIRE_FIDO2=local\|all` (#802), which forces key registration on first login (break-glass admin + service accounts exempt). Alternatively OIDC with `SPARC_OIDC_FORCE_MFA=true` delegates MFA to the IdP. Weak only where enrollment is left opt-in (`SPARC_REQUIRE_FIDO2=off`) and a user has no key — e.g. the break-glass admin path | `app/controllers/webauthn_sessions_controller.rb`, `app/controllers/concerns/authentication.rb` (enrollment gate), `app/models/sparc_config.rb` (`SPARC_FIDO2_ENABLED`, `SPARC_REQUIRE_FIDO2`, `SPARC_OIDC_FORCE_MFA`) | Implemented |
| IA-2(2) | Multi-Factor Authentication to Non-Privileged Accounts | H | Application (SPARC) | Same app-native FIDO2/WebAuthn MFA as IA-2(1) (`#779`), enforceable org-wide via `SPARC_REQUIRE_FIDO2` (#802), or OIDC IdP-enforced MFA. Weak only where enrollment is left opt-in and a user has no key | `app/controllers/webauthn_sessions_controller.rb`, `app/controllers/concerns/authentication.rb`, `SPARC_FIDO2_ENABLED`, `SPARC_REQUIRE_FIDO2` | Implemented |
...
| IA-2(8) | Access to Accounts -- Replay Resistant | H | Application (SPARC) | FIDO2/WebAuthn is inherently replay- and phishing-resistant (`#779`): per-ceremony server challenge, origin binding, and a signature counter that detects cloned keys (regression rejected). Plus session-fixation prevention via `reset_session` before storing user_id; OAuth state/nonce verification | `app/controllers/webauthn_sessions_controller.rb`, `app/controllers/concerns/authentication.rb` | Implemented |
| IA-2(12) | Acceptance of PIV Credentials | H | Hybrid | **Native PIV/CAC acceptance** (`SPARC_ENABLE_PIV`, `#779`): SPARC consumes a client certificate validated by the mTLS gateway (DoD PKI chain + revocation at the proxy/ALB, `sparc-iac#559`), parses the EDIPI/email, and maps it to a user — fail-closed unless the gateway attests verification. Because HTTP headers cannot carry newlines, gateways forward the PEM URL-encoded, newline-folded, backslash-escaped, or as bare base64 DER; `PivAuthService.normalize_pem` reassembles all of these before parsing (`#824`) so a validly presented card is not rejected as absent, and a missing certificate is audited distinctly from an undecodable one (shape-only diagnostics, never the certificate). **#822 adds a second, configurable way to satisfy the same requirement**: an OIDC token whose `acr`/`amr` claim shows the IdP itself performed certificate-based authentication (`SPARC_PIV_OIDC_ACR_VALUES` / `SPARC_PIV_OIDC_AMR_VALUES`). This avoids the shared-listener client-certificate prompt and the issuer-DN filter's inability to distinguish a hardware-bound credential from an exportable soft certificate. **Both allowlists are empty by default and empty accepts NOTHING** — a configuration mistake must not downgrade the requirement — and `acr` is compared exactly so a lower assurance level cannot satisfy a higher one | `app/controllers/piv_sessions_controller.rb`, `app/services/piv_auth_service.rb`, `app/services/piv_oidc_assertion.rb`, `SPARC_ENABLE_PIV`, `SPARC_PIV_OIDC_*`, sparc-iac | Implemented |
...
| IA-5(1) | Password-Based Authentication | H | Application (SPARC) | Minimum 12 chars; bcrypt hashing; password expiry configurable via `SPARC_PASSWORD_EXPIRY_DAYS` (default 30); `must_reset_password` flag; `password_changed_at` tracking. Administrative provisioning issues the initial authenticator rather than accepting one (#877): `password`/`password_confirmation` are not permitted attributes on either the admin UI or `POST /api/v1/users`, SPARC generates a temporary via `User#assign_temporary_password`, returns it once, and sets `must_reset_password` so it is replaced at first sign-in — satisfying the IA-5 requirement to change authenticators after initial issuance and ensuring no administrator retains knowledge of a credential the user keeps. Issuance is audited (`admin_temporary_password_issued`, `provisioning: true`). The break-glass admin is exempt from expiry (#878, see AC-2(3)) because its credential rotates out of band | `app/models/user.rb`, `app/services/user_provisioning_service.rb`, `app/controllers/api/v1/users_controller.rb`, `app/models/sparc_config.rb` | Implemented |
...
| IA-8(1) | Acceptance of PIV Credentials from Other Agencies | H | Hybrid | OIDC federation accepts cross-agency PIV. **#822** makes this concrete rather than incidental: an OIDC login satisfies the `piv` requirement in `SPARC_REQUIRE_AUTH_METHODS` when the token carries an `acr`/`amr` value the operator has explicitly accepted, so a partner agency's IdP performing certificate-based authentication is honoured without SPARC terminating mTLS itself. The satisfying claim and value are recorded as a `piv_asserted_by_idp` audit event, so an assessor can see WHY a login was accepted as PIV | `app/services/piv_oidc_assertion.rb`, `SPARC_PIV_OIDC_ACR_VALUES`, `SPARC_PIV_OIDC_AMR_VALUES` | Implemented |
...
| RA-5 | Vulnerability Monitoring and Scanning | H | Hybrid | 10 automated security scanners: Gitleaks, Brakeman, CodeQL, Semgrep, bundler-audit, importmap audit, Trivy FS, Trivy Container, CycloneDX SBOM, **Grype SBOM-driven SCA (#461)** — consumes CycloneDX SBOMs from sbom_generation + Trivy and runs vulnerability matching against Anchore's vuln DB. SARIF surfaced in Code Scanning; JSON normalized to HDF via SAF CLI `anchoregrype2hdf`. Container vulnerability baseline tracked in `docs/compliance/sparc-findings.yml` with 76 CVE dispositions, 30-day review cycle, and deployment-agnostic mitigating controls. **Continuous SCA re-scan (#616)** — `sbom-and-sca.yml` runs the org-shared Grype + Trivy fs scan (gate=CRITICAL, allow-list reconciled at `.security/sca-allowlist.yaml`) on every main push and a daily cron, so unchanged dependencies are re-evaluated against fresh CVE feeds. **Per-target attribution (#811)** — an ingested scan records the target `CdefDocument` it assessed and its `scanner_scope` (`target` vs `boundary`), and each finding carries `component_ref` / `source_location`, so tenant scan results roll up per component instead of only per authorization boundary. | `.github/workflows/security.yml`, `.github/workflows/sbom-and-sca.yml`, `app/services/hdf_ingest_service.rb`, `app/models/scan_run.rb`, `.security/sca-allowlist.yaml`, `docs/compliance/sparc-findings.yml` | Implemented |
...
| SA-9 | External System Services | H | Hybrid | OIDC/LDAP/OAuth integrations use TLS with server-certificate verification (LDAP: `VERIFY_PEER` by default, trusting the container CA store or `SPARC_LDAP_CA_FILE`); API tokens use SHA-256 digest storage; external service access audited | `app/services/ldap_auth_service.rb` | Implemented |
...
| SA-11(1) | Static Code Analysis | H | Application (SPARC) | Brakeman (Rails-specific SAST), CodeQL (semantic analysis), Semgrep (optional pattern-based SAST), **SonarCloud** (continuous SAST via the SonarCloud GitHub App). SCA augmented by Grype SBOM-driven scanning (#461) — see RA-5. **SonarCloud → OHDF evidence (#636)** — `sonarqube-hdf-emit.yml` fetches SonarCloud findings post-analysis and converts them to OASIS Heimdall Data Format (Go `hdf-cli` strategic / SAF CLI bridge), baking the HDF into the security-artifacts stream so SAST joins the unified Heimdall rollup. Vendored in-repo (`sonarqube-hdf.yml`) because sparc is public and cannot call the internal container-build-sign reusable. | `.github/workflows/security.yml`, `.github/workflows/sonarqube-hdf.yml`, `.github/workflows/sonarqube-hdf-emit.yml` | Implemented |
...
| SC-8 | Transmission Confidentiality and Integrity | H | Hybrid | `force_ssl` with HSTS (1 year, subdomains, preload); LDAP uses simple_tls or start_tls with server-certificate verification (`VERIFY_PEER`, `#773`) — the directory server's identity is authenticated, not just the channel encrypted. Outbound TLS to endpoints behind an operator-sanctioned TLS-inspecting proxy, a private CA, or DoD PKI is supported without weakening verification — verification stays ON and trusts only operator-installed CAs (an untrusted interceptor is still rejected) — via custom-CA trust (`SPARC_EXTRA_CA_CERTS` runtime mount or `certs/` build-time bake-in, `#774`); outbound calls route through a mandated egress proxy (`HTTPS_PROXY`/`NO_PROXY`, scheme-strict) via the centralized `SparcHttp` client (`#775`) | `config/environments/production.rb`, `app/services/ldap_auth_service.rb`, `bin/lib/ca-trust.sh`, `app/lib/sparc_http.rb`, `Dockerfile` | Implemented |
...
| SC-17 | Public Key Infrastructure Certificates | H | Hybrid | Inbound TLS certificate provisioning via ACM or similar (sparc-iac). Outbound trust anchors: the container trusts a custom/private CA (incl. an organization's internal or DoD-PKI root) via `SPARC_EXTRA_CA_CERTS` (runtime) or the `certs/` bake-in (build), folded into the OpenSSL trust store for all outbound clients (`#774`) | sparc-iac, `bin/lib/ca-trust.sh`, `Dockerfile`, `certs/` | Partial |
| SC-18 | Mobile Code | H | Application (SPARC) | No user-uploaded executable code; Stimulus controllers are server-managed; importmap pins JS to known versions. Enforced Content-Security-Policy restricts script execution to nonce'd/self sources and constrains `form-action`; the login page narrowly relaxes `form-action` to the enabled SSO IdP origins only, so OAuth POST-redirects succeed in Chromium while every other page keeps strict `form-action 'self'` (#593). **URL-scheme allowlist on stored hrefs (#897)** — `BackMatterResource#href` is operator-writable and rendered into anchor `href` attributes; a scheme, when present, must be `http`/`https`, blocking `javascript:`, `data:`, `vbscript:` and `file:`. Scheme-less values (OSCAL fragment references and relative paths, ~97% of real hrefs) are inert and allowed. `ApplicationHelper#safe_external_url` applies the same rule at render time for rows written before the validation existed. **Uploaded content is never served inline (#894)** — user-content downloads force `disposition: "attachment"`, asserted on the emitted signed URL rather than on a helper default. **`style-src 'unsafe-inline'` removal in progress (#1047)** — `script-src` is already nonce-restricted; `style-src` still carries `'unsafe-inline'` because ~1,400 view-level `style=` attributes would stop applying the moment it is dropped, silently and with no console error. The directive is one line and is only safe to remove when the last inline style is gone, so the work is a measured sweep rather than a config change: **1,403 → 980 attributes (30%) as of this branch**, across four screens converted to `sparc-`-prefixed theme classes, with computed values moved to `data-*` attributes set from Stimulus (a style set from JavaScript is not what `style-src` blocks). Two mechanisms hold the ground: `spec/views/inline_style_ratchet_spec.rb` is a ratchet the count may only move down through, and `tests/ui-smoke/visual_regression_1047.py` is a per-screen pixel gate. **The directive is still present and this row must not be read as closing SC-18's inline-style exposure** | `app/models/back_matter_resource.rb`, `app/helpers/application_helper.rb`, `app/controllers/concerns/artifact_resolvable.rb`, `config/importmap.rb`, `config/initializers/content_security_policy.rb`, `app/controllers/sessions_controller.rb`, `app/assets/stylesheets/sparc-theme.css`, `spec/views/inline_style_ratchet_spec.rb` | Implemented |
...
| SI-2 | Flaw Remediation | H | Hybrid | bundler-audit and Trivy detect known vulnerabilities; dependency updates via Gemfile; container rebuilds. Findings dispositioned in `sparc-findings.yml` with rationale, NIST control mapping, and **severity-based review cadence** (#244): HIGH 30d, MEDIUM 60d, LOW 120d, INFO 60d, CRITICAL 30d (POA&M only — no waivers). Disposition validation enforced by `bin/sparc_findings_to_hdf_amendments.rb`; freshness check fails CI on stale `next_review_date`. **`false_positive` exempt from the review cadence (#620)** — a false positive is a determination that the finding is not real (vulnerable code unreachable / scanner sees a present-but-not-loaded file), so it carries no risk on a remediation clock; the window/overdue checks apply only to risk-bearing `accepted`/`deferred` dispositions. Threshold-based merge gate (#244) blocks PRs that introduce new findings exceeding the scanner's band in `docs/compliance/thresholds/` after dispositions are applied. **CVE-baseline refresh (#620)** — openssl bumped to the patched bookworm-security build; Ruby default-gem CVEs (net-imap, erb, zlib) remediated by Gemfile pins (resolv-style override) so Bundler loads the patched versions while the present-but-shadowed default gems are dispositioned `false_positive`; oauth2 bumped 2.0.18→2.0.22; thruster Go-1.26.2 stdlib CVEs accepted with reverse-proxy compensating controls (no upstream build on Go≥1.26.3). **Tenant amendment generation (#449)** — `POST /api/v1/hdf/amendments_from_oscal_poam` lets tenant pipelines convert OSCAL POA&M tracker output into HDF Amendments JSON for `hdf amend apply` consumption, validated by `hdf amend verify` before serving. **HDF Amendment triage (#447)** database-backs the disposition workflow: tenants ingest scanner findings (`ScanRun` / `ScannerFinding`), triage each failed control through a UI or API into one of the seven v3.4.0 override kinds (falsePositive / waiver / poam / vendorDependency / inherited / riskAdjustment / operationalRequirement) with an evidence/POA&M/attestation/risk-assessment linkage and provenance hash, and export a deterministic HDF Amendments doc (`HdfAmendmentExportService`, validated via `hdf amend verify`) for `hdf amend apply`. The #244 severity policy is enforced against DB records (no waiver/downgrade on CRITICAL). **Remediation timelines + approval (#809)** make a disposition a time-bounded, approved decision rather than an open-ended suppression: it suppresses its finding only once approved (admin or `amendment.approve`) and only inside a validity window resolved from the boundary profile's ODP, else the admin-provisioned `RemediationTimeline` SLA grid (baseline × NIST criticality → days) measured from first-discovered; an expired amendment routes back for re-approval (see [HDF Amendment Aggregation & Lifecycle](#hdf-amendment-aggregation--lifecycle-809-811)) | `.github/workflows/security.yml`, `Gemfile`, `app/services/amendment_validity_service.rb`, `app/models/remediation_timeline.rb`, `app/services/hdf_oscal_translation_service.rb`, `app/services/hdf_ingest_service.rb`, `app/services/finding_disposition_service.rb`, `app/services/hdf_amendment_export_service.rb`, `docs/compliance/sparc-findings.yml`, `docs/compliance/thresholds/`, `bin/sparc_findings_to_hdf_amendments.rb` | Implemented |
...
| SR-3 | Supply Chain Controls and Processes | H | Hybrid | CycloneDX SBOM generation; dependency auditing; container scanning; SBOM-driven vulnerability scanning via Grype (#461) — published SBOMs are scan-verified before release, not just generated; known-good version pinning. **External CDEF content supply chain (#466)** — runtime ingestion uses GitHub blob SHA for integrity verification, ETag-conditional fetches for cache integrity, and full attribution per `docs/compliance/THIRD_PARTY_NOTICES.md` (Apache 2.0 inheritance). **License inventory and policy (#472)** — the three CycloneDX SBOMs are consolidated by `scripts/ci/build_license_inventory.rb` into a `license-inventory.{json,md}` artifact that records every component's resolved license + policy disposition + dispositions trail. Trivy's `license` scanner is enabled on both fs and container scans for full coverage. **Org-wide SCA rollup (#616)** — `sbom-and-sca.yml` wires the org-shared reusable workflows (`container-build-sign/sbom-source.yml` + `sca-scan.yml@v0.1.0`: Syft → CycloneDX, then Grype + Trivy fs reconciled against `.security/sca-allowlist.yaml`, gate=CRITICAL) and emits the source CycloneDX to `s3://<security-artifacts-bucket>/<EVIDENCE_BOUNDARY>/<YYYY-MM-DD>/sparc/sca/sparc.cdx.json` + `latest/` (GitHub-OIDC provenance) for central Dependency-Track aggregation across the org (hub-and-spoke, umbrella container-build-sign#12). Daily cron re-scans unchanged deps against fresh CVE feeds. | `.github/workflows/security.yml`, `.github/workflows/sbom-and-sca.yml`, `.security/sca-allowlist.yaml`, `Gemfile.lock`, `app/services/aws_labs_cdef_source_client.rb`, `NOTICE`, `LICENSES/AWS-LABS-OSCAL-CONTENT-LICENSE`, `scripts/ci/build_license_inventory.rb`, `docs/compliance/license-policy.yml`, `docs/compliance/license-dispositions.yml` | Implemented |
...
## HDF Amendment Aggregation & Lifecycle (#809, #811)

#447 made SPARC ingest scanner findings, triage them into dispositions, and export HDF Amendments. #811 binds each scan to the target it assessed and keeps scan history so a re-occurring finding is evaluated against its own past; #809 gates a disposition behind approval + a remediation-timeline validity window, folds the resulting evidence into the boundary's SSP/SAP/SAR/POA&M, and packages a signed bundle back to the consumer. Detailed narratives live in `component-definition-security-scanning.json`.

| Control | Contribution | CDEF | Code Location |
|---|---|---|---|
| AC-3 | Amendment approval is a distinct authorization act from triage: setting a disposition needs `evidence.write`, approving/rejecting it needs admin or the new `amendment.approve` permission key (seeded to the AO, Agency AO, and ISSM roles — the roles that accept residual risk). Every triage endpoint remains boundary-scoped | authentication, security-scanning | `app/models/role.rb` (PERMISSION_KEYS), `app/controllers/api/v1/finding_dispositions_controller.rb#authorize_approve!`, `db/seeds.rb` |
| AC-6 | Separation of duties within the disposition workflow — the decider (`decided_by`) and the approver (`approved_by`) are recorded independently, and a draft disposition has no suppressing effect until an approver acts | security-scanning, audit | `app/models/finding_disposition.rb`, `app/controllers/api/v1/finding_dispositions_controller.rb` |
| AU-10 | The signed HDF package (amendments + findings + dispositions summaries) is HMAC-SHA256 signed with an instance key derived from `SPARC_HASH` via `SparcKeyDerivation` — the same non-repudiation pattern as the #372 federation bundles, so a consumer can prove a bundle came from this SPARC instance and was not altered. Each disposition's `signature_hash` binds both decider and approver | security-scanning, session-mgmt | `app/services/hdf_package_service.rb`, `app/lib/sparc_key_derivation.rb`, `app/models/finding_disposition.rb` |
| AU-12 | Re-occurrence is never silent: every ingest writes a per-scan-run `ScannerFinding` row and demotes the prior row to `superseded` rather than overwriting it, so the full scan history — and each finding's computed lifecycle state — is an auditable record. Approve/reject and aggregation runs emit `AuditEvent` rows via the API base controller's `audit_log` | audit, security-scanning | `app/services/hdf_ingest_service.rb#record_finding`, `app/models/scanner_finding.rb`, `app/controllers/api/v1/finding_dispositions_controller.rb`, `app/controllers/api/v1/aggregations_controller.rb` |
| CA-5 | Failed findings that are **not** suppressed by an applicable, approved, in-window `falsePositive` / `waiver` / `inherited` disposition are upserted as tracked `PoamFinding` records on the boundary's POA&M (keyed `HDF: <control-id>`), so undispositioned scanner risk becomes a tracked POA&M item automatically rather than by re-keying. Idempotent — re-aggregation upserts rather than duplicates | security-scanning | `app/services/hdf_aggregation_service.rb#aggregate_poam`, `app/models/poam_finding.rb` |
| CA-7 | Continuous monitoring output reaches the authorization package instead of stopping at a worklist: `HdfAggregationService` maps each current finding to the NIST control(s) in its HDF `tags.nist` and writes a dedicated, non-destructive `hdf_scan_result` annotation field on the matching SSP / SAP / SAR controls (authored narrative is never overwritten). Runs on demand or asynchronously via `AggregateFindingsJob` | security-scanning | `app/services/hdf_aggregation_service.rb`, `app/jobs/aggregate_findings_job.rb`, `app/controllers/api/v1/aggregations_controller.rb` |
| CA-7(4) | Risk monitoring across scans: a finding that was passing or dispositioned and then fails again — or fails at a worse severity — is classified `re_failed` and surfaced distinctly on the triage board, separating regression from steady-state known risk | security-scanning | `app/services/hdf_ingest_service.rb#lifecycle_for`, `app/controllers/hdf_triage_controller.rb` |
| RA-3 | An amendment is a time-bounded risk decision, not a permanent suppression. `AmendmentValidityService` computes `valid_until` from the boundary profile's baseline level × the finding's NIST criticality, measured from the control's **first-discovered** scan; an open POA&M addressing the control confers validity for as long as it stays open. An expired amendment stops suppressing and the finding returns to the board as `expired` for re-approval | security-scanning | `app/services/amendment_validity_service.rb`, `app/models/remediation_timeline.rb` |
| RA-5 | Scans are attributable to what they assessed: ingest records the target `CdefDocument` and a `scanner_scope` (`target` vs `boundary`), and each finding carries `component_ref` / `source_location`, so vulnerability results roll up per component rather than only per boundary | security-scanning | `app/services/hdf_ingest_service.rb`, `app/models/scan_run.rb`, `app/models/scanner_finding.rb` |
| SI-2 | Flaw-remediation timelines are enforced by data, not convention: the admin-provisioned `RemediationTimeline` SLA grid (baseline Low/Moderate/High × NIST criticality Critical/High/Moderate/Low/Informational/Unknown → days) supplies the remediation window whenever the boundary profile carries no ODP value, and is managed through an admin screen and `Api::V1::Admin` endpoints with seeded defaults | security-scanning | `app/models/remediation_timeline.rb`, `app/controllers/admin/remediation_timelines_controller.rb`, `app/controllers/api/v1/admin/remediation_timelines_controller.rb`, `db/seeds.rb` |

**Configuration dependencies:**
- `SPARC_HASH` (≥32 bytes) keys the HDF package signature via `SparcKeyDerivation`; without it the derivation falls back to Rails `secret_key_base` (logged warning in production), which weakens the cross-instance provenance claim on the bundle.
- The `RemediationTimeline` SLA grid seeds with defaults but is intended to be reviewed and set by the Instance Admin to the organization's actual remediation policy — the seeded values are a starting point, not an organizational determination.
- Amendment approval falls back to Instance Admins wherever no role has been granted `amendment.approve`.

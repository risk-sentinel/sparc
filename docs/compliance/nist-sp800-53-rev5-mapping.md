...
**System:** SPARC (Systemized Policy and Regulatory Controls)
...
| AC-2 | Account Management | H | Application (SPARC) | User model with active/suspended/deactivated statuses; admin bootstrap; role assignment; organization membership | `app/models/user.rb` | Implemented |
...
| AU-12 | Audit Record Generation | H | Application (SPARC) | `AuditEvent.log` factory called throughout controllers and models; `audit_log` helper in API base controller generates records for all document CRUD mutations. Phase 2 API extends `audit_log()` coverage to catalog, profile, CDEF, and control-mapping controllers for create/update/destroy actions. Baseline parameter updates are audit-logged via `audit_log()` in `BaselineParametersController` | `app/models/audit_event.rb`, `app/controllers/api/v1/base_controller.rb`, `app/controllers/api/v1/baseline_parameters_controller.rb` | Implemented |
...
| CA-2 | Control Assessments | H | Hybrid | SPARC manages SAR documents for assessment results; automated security scanning provides continuous evidence | `app/models/sar_document.rb`, `.github/workflows/security.yml` | Implemented |
...
| CA-7 | Continuous Monitoring | H | Hybrid | Weekly scheduled security scans; HDF normalization enables dashboard integration; audit event stream | `.github/workflows/security.yml`, `app/models/audit_event.rb` | Implemented |
...
| CA-7(4) | Risk Monitoring | H | Hybrid | Security pipeline evaluates severity thresholds; dependency audit detects new CVEs | `.github/workflows/security.yml` | Implemented |
...
| CM-3 | Configuration Change Control | H | Hybrid | Pull request workflow with required reviews; CI security gates; Terraform plan/apply in sparc-iac | `.github/workflows/`, sparc-iac | Implemented |
...
| CM-6 | Configuration Settings | H | Hybrid | `SparcConfig` centralizes all SPARC_* env var settings with secure defaults; infrastructure config in sparc-iac. New: `SPARC_API_OIDC_AUDIENCE` for JWT audience validation. `BaselineParameterService` enables API-driven baseline parameter customization for profile documents | `app/models/sparc_config.rb`, `app/services/baseline_parameter_service.rb`, `app/controllers/api/v1/baseline_parameters_controller.rb` | Implemented |
...
| CM-8 | System Component Inventory | H | Hybrid | CycloneDX SBOM generation for Ruby dependencies; Trivy CycloneDX for container components | `.github/workflows/security.yml` (sbom_generation job) | Implemented |
...
| IA-2 | Identification and Authentication (Organizational Users) | H | Application (SPARC) | Multi-provider authentication: local (bcrypt), OIDC, LDAP, GitHub OAuth, GitLab OAuth. API supports dual auth: SPARC Bearer tokens (SHA-256 digest) and Okta JWT introspection (RS256). **⚠ CONDITIONAL:** Full coverage (incl. MFA) requires `SPARC_ENABLE_OIDC=true` + `SPARC_OIDC_FORCE_MFA=true`. Local-only login = single-factor only | `app/models/sparc_config.rb`, `app/controllers/concerns/authentication.rb`, `app/controllers/concerns/api_authentication.rb` | Implemented |
...
| IA-4 | Identifier Management | H | Application (SPARC) | UUID immutability enforced (`enforce_uuid_immutability` callback); email uniqueness validation; case-insensitive normalization | `app/models/user.rb` | Implemented |
...
| IA-5 | Authenticator Management | H | Application (SPARC) | bcrypt password hashing via `has_secure_password`; minimum 12 characters (NIST 800-63B); password expiry; forced reset. API tokens use SHA-256 digest; OIDC JWT tokens validated via RS256 signature verification | `app/models/user.rb`, `app/models/api_token.rb`, `app/controllers/concerns/api_authentication.rb` | Implemented |
...
| RA-3 | Risk Assessment | H | Hybrid | Automated vulnerability scanning provides risk data; SAR documents capture assessment results | `app/models/sar_document.rb`, `.github/workflows/security.yml` | Implemented |
| RA-3(1) | Supply Chain Risk Assessment | H | Hybrid | SBOM generation; dependency vulnerability scanning; supply chain analysis | `.github/workflows/security.yml` (sbom_generation, dependency_audit) | Implemented |
| RA-5 | Vulnerability Monitoring and Scanning | H | Hybrid | 9 automated security scanners: Gitleaks, Brakeman, CodeQL, Semgrep, bundler-audit, importmap audit, Trivy FS, Trivy Container, CycloneDX | `.github/workflows/security.yml` | Implemented |
| RA-5(2) | Update Vulnerabilities to Be Scanned | H | Hybrid | bundler-audit updates advisory database before each scan; Trivy uses latest vulnerability database | `.github/workflows/security.yml` | Implemented |
...
| SA-11 | Developer Testing and Evaluation | H | Hybrid | RSpec test suite; Brakeman SAST; CodeQL semantic analysis; security scan pipeline | `spec/`, `.github/workflows/security.yml` | Implemented |
| SA-11(1) | Static Code Analysis | H | Application (SPARC) | Brakeman (Rails-specific SAST), CodeQL (semantic analysis), Semgrep (optional pattern-based SAST) | `.github/workflows/security.yml` | Implemented |
| SA-15 | Development Process, Standards, and Tools | H | Hybrid | rubocop-rails-omakase linting; RSpec testing framework; documented development workflow | `.rubocop.yml`, `spec/` | Implemented |
...
| SC-12 | Cryptographic Key Establishment and Management | H | Hybrid | bcrypt cost factor for passwords; SHA-256 for API tokens; TLS certificate management in sparc-iac | `app/models/user.rb`, sparc-iac | Implemented |
...
| SC-18 | Mobile Code | H | Application (SPARC) | No user-uploaded executable code; Stimulus controllers are server-managed; importmap pins JS to known versions | `config/importmap.rb` | Implemented |
...
| SC-28 | Protection of Information at Rest | H | Infrastructure (sparc-iac) | RDS encryption; S3 server-side encryption; EBS encryption | sparc-iac | Planned |
...
| SI-2 | Flaw Remediation | H | Hybrid | bundler-audit and Trivy detect known vulnerabilities; dependency updates via Gemfile; container rebuilds | `.github/workflows/security.yml`, `Gemfile` | Implemented |
...
| SI-10 | Information Input Validation | H | Application (SPARC) | Rails strong parameters; model validations (email format, password length, status inclusion); XSS sanitization for banner HTML | `app/models/user.rb`, Rails defaults | Implemented |
...
| SR-3 | Supply Chain Controls and Processes | H | Hybrid | CycloneDX SBOM generation; dependency auditing; container scanning; known-good version pinning | `.github/workflows/security.yml`, `Gemfile.lock` | Implemented |
...
| SR-11 | Component Authenticity | H | Hybrid | Gemfile.lock with specific version pins; container image provenance; SBOM tracks component origins | `Gemfile.lock`, `.github/workflows/security.yml` | Implemented |

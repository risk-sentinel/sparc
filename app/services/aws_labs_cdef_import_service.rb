...
class AwsLabsCdefImportService
  # #1103 — CciNistResolvable was included here for `nist_family_from_id`, which
  # left with the enrichment. CdefNistEnrichmentService includes it now, so the
  # three importers still cannot drift on what a family is.
  ...
  #
  # #1103 — the implementation moved to CdefNistEnrichmentService. It lived here
  # as a private method, which meant it ran ONLY on this service's path: a CDEF
  # uploaded through the UI parsed its controls and never resolved a single one
  # to NIST. Both entry points now call the same service.
  def enrich_with_nist_mappings!(document)
    CdefNistEnrichmentService.new(logger: @logger).enrich!(document)
  end

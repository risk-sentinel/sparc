...
class OscalSspExportService
  ...
  def build_implemented_requirement(control)
    ...
    # Props carry status, type_use_as, provided_as, control_origination
  ...
  def build_props(field_map)
    ...
    type_use = field_map["type_use_as"]&.field_value
    props << { "name" => "control-type", "ns" => "https://sparc.local/ns", "value" => type_use } if type_use.present?

    provided_as = field_map["provided_as"]&.field_value
    props << { "name" => "provided-as", "ns" => "https://sparc.local/ns", "value" => provided_as } if provided_as.present?

    origination = field_map["control_origination"]&.field_value
  ...
  def build_statements(control, field_map)
    ...
    priv = field_map["private_implementation"]&.field_value
    if priv.present?
      statements << {
    ...
    pub = field_map["public_implementation"]&.field_value
    if pub.present?
      statements << {
    ...
    control.provider_statements.each_with_index do |stmt, i|
      ...
      priv_impl = stmt_fields["private_implementation"]&.field_value
      pub_impl  = stmt_fields["public_implementation"]&.field_value

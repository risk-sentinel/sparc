...
#
# The client cert carries both identity shapes so one card exercises every
...
openssl x509 -req -in "$OUT/client.csr" -CA "$OUT/ca.crt" -CAkey "$OUT/ca.key" \
  ...
  -extfile <(printf 'subjectAltName=email:john.doe@mail.mil,otherName:%s;UTF8:%s@mil' "$UPN_OID" "$EDIPI") 2>/dev/null
